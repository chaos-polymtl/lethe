...
template <int dim>
class DirectSteadyGLSNavierStokes
{
public:
  DirectSteadyGLSNavierStokes(const unsigned int degreeVelocity,
                              const unsigned int degreePressure);
  ~DirectSteadyGLSNavierStokes();
  void
  runMMS();
  void
  runCouette();
...
// Constructor
template <int dim>
DirectSteadyGLSNavierStokes<dim>::DirectSteadyGLSNavierStokes(
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::make_cube_grid(int refinementLevel)
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::refine_grid()
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::setup_dofs()
{
  system_matrix.clear();
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::initialize_system()
{
  {
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::assemble(const bool initial_step,
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::assemble_system(const bool initial_step)
{
  assemble(initial_step, true);
}
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::assemble_rhs(const bool initial_step)
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::solve(const bool initial_step)
{
  const AffineConstraints<double> &constraints_used =
    initial_step ? nonzero_constraints : zero_constraints;
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::refine_mesh()
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::refine_mesh_uniform()
{
  SolutionTransfer<dim, BlockVector<double>> solution_transfer(dof_handler);
  solution_transfer.prepare_for_coarsening_and_refinement(present_solution);
  triangulation.refine_global(1);
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::newton_iteration(
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::output_results(const unsigned int cycle) const
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::calculateL2Error()
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::runMMS()
{
...
template <int dim>
void
DirectSteadyGLSNavierStokes<dim>::runCouette()
{
  viscosity_ = 10;
  GridIn<dim> grid_in;
...
main()
{
  try
    {
      DirectSteadyGLSNavierStokes<2> problem_2d(1, 1);
