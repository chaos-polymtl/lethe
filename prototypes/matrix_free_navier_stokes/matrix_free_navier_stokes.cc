...
Settings::try_parse(const std::string &prm_filename)
{
  ...
  prm.declare_entry("relaxation parameter",
                    "0.5",
                    Patterns::Double(),
                    "Relaxation for smoother in LSMG and GCMG");
  ...
  this->relaxation_parameter = prm.get_double("relaxation parameter");
                                      ...
                                      Vector<double> &  values) const
{
  ...
  // 2D Navier-Stokes analytical solution
  // u=sin(a*x)*sin(a*x)*cos(a*y)*sin(a*y)
  // v=-cos(a*x)*sin(a*x)*sin(a*y)*sin(a*y)
  // p=0
  // 3D Navier-Stokes analytical solution
  ...
  if constexpr (dim == 3)
    {
      const double z = p[2];
      values(0)      = std::sin(a * x) * std::sin(a * x) * std::cos(a * y) *
                  std::sin(a * y) * std::cos(a * z) * std::sin(a * z);
      values(1) = std::cos(a * x) * std::sin(a * x) * std::sin(a * y) *
                  std::sin(a * y) * std::cos(a * z) * std::sin(a * z);
      values(2) = -2 * std::cos(a * x) * std::sin(a * x) * std::cos(a * y) *
                  std::sin(a * y) * std::sin(a * z) * std::sin(a * z);
      values(3) = x * x + y * y + z * z;
    }
}
      ...
      if (component == 0)
        {
          return (2 * a * a *
                  ...
                  4 * a * a * std::sin(a * x) * std::sin(a * x) *
                    std::sin(a * y) * std::cos(a * y) +
                  2 * a * std::sin(a * x) * std::sin(a * x) * std::sin(a * x) *
                    std::sin(a * y) * std::sin(a * y) * std::cos(a * x) *
                    std::cos(a * y) * std::cos(a * y) -
                  (-a * std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                     std::sin(a * y) +
                   a * std::sin(a * x) * std::sin(a * x) * std::cos(a * y) *
                     std::cos(a * y)) *
                    std::sin(a * x) * std::sin(a * y) * std::sin(a * y) *
                    std::cos(a * x));
        }
      else if (component == 1)
        {
          return (-2 * a * a *
                  ...
                  2 * a * std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                    std::sin(a * y) * std::sin(a * y) * std::cos(a * x) *
                    std::cos(a * x) * std::cos(a * y) +
                  (a * std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                     std::sin(a * y) -
                   a * std::sin(a * y) * std::sin(a * y) * std::cos(a * x) *
                     std::cos(a * x)) *
                    std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                    std::cos(a * y));
        }
      else
        {
  ...
  if constexpr (dim == 3)
    {
      ...
      if (component == 0)
        {
          return (2 * a * a * (2 - 3 * std::cos(2 * a * x)) * std::sin(a * y) *
                    std::sin(a * z) * std::cos(a * y) * std::cos(a * z) +
                  a *
                    (2 * std::cos(a * y) * std::cos(a * y) -
                     std::cos(a * z) * std::cos(a * z)) *
                    std::sin(a * x) * std::sin(a * x) * std::sin(a * x) *
                    std::sin(a * y) * std::sin(a * y) * std::sin(a * z) *
                    std::sin(a * z) * std::cos(a * x) +
                  2 * x);
        }
      else if (component == 1)
        {
          return (2 * a * a * (2 - 3 * std::cos(2 * a * y)) * std::sin(a * x) *
                    std::sin(a * z) * std::cos(a * x) * std::cos(a * z) +
                  a *
                    (2 * std::cos(a * x) * std::cos(a * x) -
                     std::cos(a * z) * std::cos(a * z)) *
                    std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                    std::sin(a * y) * std::sin(a * y) * std::sin(a * z) *
                    std::sin(a * z) * std::cos(a * y) +
                  2 * y);
        }
      else
        {
          return (4 * a * a * (3 * std::cos(2 * a * z) - 2) * std::sin(a * x) *
                    std::sin(a * y) * std::cos(a * x) * std::cos(a * y) +
                  2 * a *
                    (std::cos(a * x) * std::cos(a * x) +
                     std::cos(a * y) * std::cos(a * y)) *
                    std::sin(a * x) * std::sin(a * x) * std::sin(a * y) *
                    std::sin(a * y) * std::sin(a * z) * std::sin(a * z) *
                    std::sin(a * z) * std::cos(a * z) +
                  2 * z);
        }
    }
}
...
template <int dim, typename number>
const std::shared_ptr<const Utilities::MPI::Partitioner> &
NavierStokesOperator<dim, number>::get_vector_partitioner() const
{
  ...
  FECellIntegrator &integrator) const
{
  ...
  auto h = integrator.read_cell_data(element_size);
  ...
  if constexpr (dim == 3)
    {
      ...
      dirichlet_boundary_ids.insert(5);
    }
...
private:
  ...
  using SystemMatrixType = NavierStokesOperator<dim, double>;
  SystemMatrixType  system_matrix;
  MGConstrainedDoFs mg_constrained_dofs;
  using LevelMatrixType = NavierStokesOperator<dim, double>;
  ...
  for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      auto h = phi.read_cell_data(system_matrix.get_element_size());
...
MatrixFreeNavierStokes<dim>::compute_update()
{
  ...
  system_matrix.evaluate_non_linear_term(solution);
...
{
  {
    ...
    std::string VISCOSITY_header =
      "Kinematic viscosity: " + std::to_string(parameters.viscosity);
    std::string PRECOND_header  = "";
    std::string SMOOTHER_header = "";
    if (parameters.preconditioner == Settings::amg)
      {
        PRECOND_header  = "Preconditioner: AMG";
        SMOOTHER_header = "Relaxation parameter for smoother: NA";
      }
    else if (parameters.preconditioner == Settings::gcmg)
      {
        PRECOND_header  = "Preconditioner: GCMG";
        SMOOTHER_header = "Relaxation parameter for smoother: " +
                          std::to_string(parameters.relaxation_parameter);
      }
    ...
    pcout << VISCOSITY_header << std::endl;
    pcout << PRECOND_header << std::endl;
    pcout << SMOOTHER_header << std::endl;
