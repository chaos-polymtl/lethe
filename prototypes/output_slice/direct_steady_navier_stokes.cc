...
private:
  ...
  Triangulation<dim-1, dim> patch_tria;
  MappingQ<dim-1, dim> patch_mapping;
  FESystem<dim>      fe;
...
DirectSteadyNavierStokes<dim>::DirectSteadyNavierStokes(
  ...
  , patch_mapping(degreeVelocity)
  , mapping(degreeVelocity)
{}
...
template <int dim>
void
DirectSteadyNavierStokes<dim>::output_slice_results(const unsigned int cycle)
{
  GridGenerator::subdivided_hyper_cube(patch_tria, 100, -2., 2., false);
  ...
  DataOutResample<3, 2, 3> data_out_slice(patch_tria, patch_mapping);
  data_out_slice.attach_dof_handler(dof_handler);
  data_out_slice.add_data_vector(present_solution,
                                 solution_names,
                                 DataOutResample<3,2,3>::type_dof_data,
                                 data_component_interpretation);
  data_out_slice.build_patches(mapping);
  std::string basic_string = "solution-slice-";
  basic_string += ('0' + cycle);
  basic_string += ".vtk";
  std::cout << "Writing file : " << basic_string << std::endl;
  std::ofstream outputSolution(basic_string.c_str());

  data_out_slice.write_vtk(outputSolution);
}
...
main()
{
  try
    {
      DirectSteadyNavierStokes<3> problem_3d(2, 1);
      //problem_3d.runCouette();
      problem_3d.runMMS();
    }
