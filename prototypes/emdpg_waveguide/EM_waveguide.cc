...
// This is a helper function that maps a 3D tensor to a 2D face in H^-1/2(curl).
template <int dim>
...
template <int dim> class Parameters : public ParameterAcceptor {
public:
  // CONSTRUCTOR
  Parameters();
...
template <int dim>
Parameters<dim>::Parameters() : ParameterAcceptor("Parameters") {
  ...
  add_parameter("waveguide length", waveguide_length,
  ...
  delta_degree = 1;
  add_parameter("delta degree", delta_degree,
 ...
 * time convention with the imaginary unit in the exponent being negative (i.e.
 * exp(-i wt) ). The function manage both the real and complex components of the
 * solution. Since the frequency, the material properties, and the geometry are
 * arguments for the parameter file, they are used in the constructor to
 * initialize the different wavenumber. Note that everything is addimensionalize
 * using the following :
 *       \tilde{\mathbf{E}} = \frac{\mathbf{E}}{E_0},
 *       \tilde{\mathbf{H}} = \frac{Z_0}{E_0} \mathbf{H},
...
template <int dim> class AnalyticalSolution_E : public Function<dim> {
public:
  // Constructor
  AnalyticalSolution_E(const Parameters<dim> &parameters)
      : Function<dim>(6), parameters(parameters),
...
template <int dim>
double AnalyticalSolution_E<dim>::value(const Point<dim> &p,
  ...
  if (component == 0)
    return (-factor * k_y * std::cos(k_x * p[0]) * std::sin(k_y * p[1])).real();
  else if (component == 1)
    return (factor * k_x * std::sin(k_x * p[0]) * std::cos(k_y * p[1])).real();
  else if (component == 2)
    return 0.;
  else if (component == 3)
    return (-factor * k_y * std::cos(k_x * p[0]) * std::sin(k_y * p[1])).imag();
  else if (component == 4)
    return (factor * k_x * std::sin(k_x * p[0]) * std::cos(k_y * p[1])).imag();
  else if (component == 5)
    return 0.;
  else
    throw std::runtime_error("Too many components for the analytical solution");
}

// Same here, we create the analytical solution class for the magnetic field
// (H).
template <int dim> class AnalyticalSolution_H : public Function<dim> {
public:
  // Constructor
...
template <int dim>
double AnalyticalSolution_H<dim>::value(const Point<dim> &p,
                                        const unsigned int component) const {
...
template <int dim> class Time_Harmonic_Maxwell_DPG : public ParameterAcceptor {
public:
...
private:
  ...
  void make_grid(const unsigned int initial_refinement);

  // <code>refine_grid</code> refines the mesh uniformly or adaptively using the
  // built-in error estimator depending on the input parameters. Note that at
  // the moment the adaptive refinement does not work in parallel for
  // fe_nedelecSZ elements. From quick tests, it seems to be related to hanging
  // nodes constraints. Use the standard Nedelec elements if you want to use
...
// $\Im(\hat{E}_\parallel)$, $\Re(\hat{H}_\parallel)$, $\Im(\hat{H}_\parallel)$;
...
Time_Harmonic_Maxwell_DPG<dim>::Time_Harmonic_Maxwell_DPG(
    const Parameters<dim> &parameters)
    : ParameterAcceptor("Time_Harmonic_Maxwell_DPG"), parameters(parameters),
      k(2. * M_PI * parameters.frequency / parameters.speed_of_light),
      k_z(std::sqrt(
          k * k -
      ...
      fe_trial_interior(FE_DGQ<dim>(parameters.degree) ^ dim,
                        ...
                        FE_DGQ<dim>(parameters.degree) ^ dim,
                        FE_DGQ<dim>(parameters.degree) ^
                            dim), // (E, E_imag, H, H_imag)
      ...
      fe_trial_skeleton(
          ...
          FE_NedelecSZ<dim>(parameters.degree),
          FE_NedelecSZ<dim>(
              parameters.degree)), // (E_hat, E_hat_imag, H_hat, H_hat_imag)
      ...
      fe_test(
          ...
          FE_NedelecSZ<dim>(parameters.degree + parameters.delta_degree),
          FE_NedelecSZ<dim>(parameters.degree +
                            parameters.delta_degree)), // (F, F_imag, I, I_imag)
      dof_handler_test(triangulation), extractor_E_real(0),
      extractor_E_imag(dim), extractor_H_real(2 * dim),
  ...
  AssertThrow(k > 0, ExcMessage("The wavenumber must be positive, its the "
                                "magnitude of the wave vector."));
}

// Here the <code>make_grid</code> function creates the initial rectangular mesh
// using a structured grid using
// dealii::GridGenerator::subdivided_hyper_rectangle.
template <int dim>
void Time_Harmonic_Maxwell_DPG<dim>::make_grid(
  ...
  GridGenerator::subdivided_hyper_rectangle(
      triangulation, parameters.n_repetitions, lower_left_waveguide,
      upper_right_waveguide, true);
...
// solution.
template <int dim> void Time_Harmonic_Maxwell_DPG<dim>::refine_grid() {
...
// In this function, we initialize the three different DoFHandlers and apply the
// constraints related to the boundary conditions. We also create the sparsity
// pattern and initialize the system matrix and right-hand side.
template <int dim> void Time_Harmonic_Maxwell_DPG<dim>::setup_system() {
  ...
  pcout << std::endl
        << "Number of dofs for the interior: " << n_dofs_interior << std::endl;

  pcout << "Number of dofs for the skeleton: " << n_dofs_skeleton << std::endl;
  ...
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, 0, Functions::ZeroFunction<dim>(4 * dim), 0,
      constraints);
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, dim, Functions::ZeroFunction<dim>(4 * dim), 0,
      constraints);

  // x boundary n x E = 0
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, 0, Functions::ZeroFunction<dim>(4 * dim), 1,
      constraints);
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, dim, Functions::ZeroFunction<dim>(4 * dim), 1,
      constraints);

  // -y boundary n x E = 0
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, 0, Functions::ZeroFunction<dim>(4 * dim), 2,
      constraints);
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, dim, Functions::ZeroFunction<dim>(4 * dim), 2,
      constraints);

  // y boundary n x E = 0
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, 0, Functions::ZeroFunction<dim>(4 * dim), 3,
      constraints);
  VectorTools::project_boundary_values_curl_conforming_l2(
      dof_handler_trial_skeleton, dim, Functions::ZeroFunction<dim>(4 * dim), 3,
      constraints);
  ...
  std::vector<bool> is_dof_on_face(fe_trial_skeleton.n_dofs_per_cell(), false);
    ...
    if (cell->is_locally_owned()) {
      ...
      for (const auto &face : cell->face_iterators()) {
        ...
        for (const auto &face_dof : face_dof_indices) {
          ...
          // Find the first iterator in the cell dof indices that matches the
          // face dof (find where the face dof is in the cell dof indices)
          const auto it = std::find(cell_dof_indices.begin(),
                                    cell_dof_indices.end(), face_dof);

          // If the dof is on the face (find returns the second iterator if no
          // match is found), set the corresponding flag to true
          if (it != cell_dof_indices.end()) {
            is_dof_on_face[std::distance(cell_dof_indices.begin(), it)] = true;
          }
        }
      }

      // Loop on all dofs on the cell and constrain the interior ones to zero
  ...
  // associated with the skeleton unknowns. We also initialize all the parallel
  // vector that will contain the solution (interior and on the skeleton) and
  ...
  SparsityTools::distribute_sparsity_pattern(
      dsp, locally_owned_dofs_trial_skeleton, mpi_communicator,
      locally_relevant_dofs_trial_skeleton);

  system_matrix.reinit(locally_owned_dofs_trial_skeleton,
                       locally_owned_dofs_trial_skeleton, dsp,
                       mpi_communicator);

  locally_relevant_solution_skeleton.reinit(
      locally_owned_dofs_trial_skeleton, locally_relevant_dofs_trial_skeleton,
      mpi_communicator);
  system_rhs.reinit(locally_owned_dofs_trial_skeleton, mpi_communicator);
  locally_relevant_solution_interior.reinit(
      locally_owned_dofs_trial_interior, locally_relevant_dofs_trial_interior,
      mpi_communicator);
  locally_relevant_residual_estimator.reinit(
      locally_owned_dofs_test, locally_relevant_dofs_test, mpi_communicator);
};

// Here we assemble the system matrix and right-hand side and apply the Robin
// boundary conditions during the assembly.
template <int dim>
void Time_Harmonic_Maxwell_DPG<dim>::assemble_system(
    const bool solve_interior) {
  ...
  // To avoid unecessary computations, we will precompute the shape functions of
  // all our spaces once and then use those precomputed values to assemble the
  // local matrices. Consequently, we need to create containers to store these
  // values. Since those are complex-valued functions, we use two different
  // containers for the real and imaginary parts because the conjugate of a
  // complex tensor is not implemented in deal.II.
  ...
  // Also, to avoid multiple if calls to understand where assemble each term in
  // the matrix, we will use containers to store dofs relationships for each of
  // the local matrices.
  ...
  // B matrix for the bilinear form and relationships between interior trial and
  // test
  ...
  // B_hat matrix for the bilinear form and relationships between skeleton trial
  // and test
  ...
  // We define some constants that will be used during the assembly. Those would
  // change according to the material parameters, but here we only have one
  // material.
  ...
  for (const auto &cell : dof_handler_trial_interior.active_cell_iterators()) {
    ...
    if (cell->is_locally_owned()) {
      ...
      // We fill the dofs relationship containers at the cell level. To do so,
      // we first loop on the test space dofs.
      for (unsigned int i : fe_values_test.dof_indices()) {
        ...
        if ((current_element_test_i == 0) || (current_element_test_i == 1)) {
          l_F.emplace_back(i);
        }

        // Loop over the dofs test a second time to fill the dofs relationship
        // for the G matrix (Riesz map)
        ...
        // Then we loop over the dofs trial space to fill the dofs relationship
        // for the B matrix (bilinear form)
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
        ...
        // containers for the real and imaginary parts of the electric and
        // magnetic fields and the Dof relationship at the current quadrature
        // point.
        const double &JxW = fe_values_trial_interior.JxW(q_point);

        for (unsigned int i : fe_values_test.dof_indices()) {
          F[i] = fe_values_test[extractor_E_real].value(i, q_point) +
                 imag * fe_values_test[extractor_E_imag].value(i, q_point);
          F_conj[i] = fe_values_test[extractor_E_real].value(i, q_point) -
                      imag * fe_values_test[extractor_E_imag].value(i, q_point);

          curl_F[i] = fe_values_test[extractor_E_real].curl(i, q_point) +
                      imag * fe_values_test[extractor_E_imag].curl(i, q_point);
          curl_F_conj[i] =
              fe_values_test[extractor_E_real].curl(i, q_point) -
              imag * fe_values_test[extractor_E_imag].curl(i, q_point);

          I[i] = fe_values_test[extractor_H_real].value(i, q_point) +
                 imag * fe_values_test[extractor_H_imag].value(i, q_point);
          I_conj[i] = fe_values_test[extractor_H_real].value(i, q_point) -
                      imag * fe_values_test[extractor_H_imag].value(i, q_point);

          curl_I[i] = fe_values_test[extractor_H_real].curl(i, q_point) +
        ...
        for (unsigned int i : fe_values_trial_interior.dof_indices()) {
          E[i] = fe_values_trial_interior[extractor_E_real].value(i, q_point) +
                 imag * fe_values_trial_interior[extractor_E_imag].value(
                            i, q_point);
          H[i] = fe_values_trial_interior[extractor_H_real].value(i, q_point) +
                 imag * fe_values_trial_interior[extractor_H_imag].value(
                            i, q_point);
        }

        // Now we loop on each relationship container to assemble the relevant
        // matrices
        ...
        for (const auto &[i, j] : G_IF) {
          G_matrix(i, j) += (((conj_ikepsilon_Zr * F[j] * curl_I_conj[i]) -
        ...
        for (const auto &[i, j] : B_FE) {
          B_matrix(i, j) += (ikepsilon_Zr * E[j] * F_conj[i] * JxW).real();
        }
      ...
      for (const auto &face : cell_skeleton->face_iterators()) {
        ...
        // We fill the dofs relationship containers at the face level. To do so,
        // we first loop on the test space dofs.
        for (unsigned int i : fe_face_values_test.dof_indices()) {
          ...
          // If we are at either boundary 4 or 5, we are on the Robin B.C. and
          // the load vector and the B_hat matrix will have a contribution in
          // addition to a modification of the riesz map (G matrix) because of
          // the energy norm that we want to minimize there.
          if ((face->boundary_id() == 4) || (face->boundary_id() == 5)) {
            ...
            // Loop over the dofs test to fill the G_matrix dofs relationship
            ...
            // Loop over the dofs trial space to fill the B_hat matrix dofs
            // relationship for the Robin boundary condition
            for (unsigned int j : fe_face_values_trial_skeleton.dof_indices()) {
            ...
            // If not on Robin B.C., assemble all the other relevantskeleton
            // terms
            for (unsigned int j : fe_face_values_trial_skeleton.dof_indices()) {
              const unsigned int current_element_trial_j =
                  fe_trial_skeleton.system_to_base_index(j).first.first;
        ...
        for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point) {
          ...
          const double JxW_face = fe_face_values_trial_skeleton.JxW(q_point);

          // As for the cell, we first loop over the test dofs to fill the face
          // values containers
          for (unsigned int i : fe_face_values_test.dof_indices()) {
            F_face[i] =
                fe_face_values_test[extractor_E_real].value(i, q_point) +
                imag * fe_face_values_test[extractor_E_imag].value(i, q_point);
            F_face_conj[i] =
                fe_face_values_test[extractor_E_real].value(i, q_point) -
                imag * fe_face_values_test[extractor_E_imag].value(i, q_point);

            I_face_conj[i] =
                fe_face_values_test[extractor_H_real].value(i, q_point) -
                imag * fe_face_values_test[extractor_H_imag].value(i, q_point);
          ...
          // Then, similarly we loop over the trial dofs to fill the face values
          // containers. Note that the to be in H^-1/2(curl), the fields needs
          // to have the tangential property mapping (n x (E x n)) which
          // effectively extract the tangential component of the field at the
          // face. So here we apply this operation using the map_H12 function
          // that we defined earlier. Stricly speeking, nx(E_parallel) = n x E,
          // and we would not need to use the map_H12 function, but we keep it
          // for consistency.
          for (unsigned int i : fe_face_values_trial_skeleton.dof_indices()) {

            E_hat[i] = map_H12(
                fe_face_values_trial_skeleton[extractor_E_real].value(i,
                                                                      q_point) +
                    imag *
                        fe_face_values_trial_skeleton[extractor_E_imag].value(
                            i, q_point),
                normal);

            n_cross_E_hat[i] = cross_product_3d(
                normal,
                map_H12(fe_face_values_trial_skeleton[extractor_E_real].value(
                            i, q_point) +
                            imag *
            ...
            n_cross_H_hat[i] = cross_product_3d(
                normal,
                map_H12(fe_face_values_trial_skeleton[extractor_H_real].value(
                            i, q_point) +
                            imag *
          ...
          // Here we get the excitation at the inlet port. To do so we force the
          // electromagnetic fields with our analytical solution at the
          // boundary.
          if (face->boundary_id() == 4) {

            H_inc[0] = analytical_solution_H.value(position, 0) +
                       imag * analytical_solution_H.value(position, 3);
            H_inc[1] = analytical_solution_H.value(position, 1) +
                       imag * analytical_solution_H.value(position, 4);
            H_inc[2] = analytical_solution_H.value(position, 2) +
                       imag * analytical_solution_H.value(position, 5);

            E_inc[0] = analytical_solution_E.value(position, 0) +
                       imag * analytical_solution_E.value(position, 3);
            E_inc[1] = analytical_solution_E.value(position, 1) +
                       imag * analytical_solution_E.value(position, 4);
            E_inc[2] = analytical_solution_E.value(position, 2) +
                       imag * analytical_solution_E.value(position, 5);

            g_inc = cross_product_3d(normal, H_inc) +
                    map_H12(kz_kZr * E_inc, normal);
          }
          // If we are at the outlet, we want to have an absorbing boundary
          // condition so we set the excitation to zero.
          ...
          for (const auto &[i, j] : G_FF) {
            G_matrix(i, j) +=
                (conj_kz_kZr * F_face[j] * kz_kZr * F_face_conj[i] * JxW_face)
                    .real();
          }

          for (const auto &[i, j] : G_FI) {
            G_matrix(i, j) +=
                (n_cross_I_face[j] * kz_kZr * F_face_conj[i] * JxW_face).real();
          }

          for (const auto &[i, j] : G_IF) {
            G_matrix(i, j) +=
                (conj_kz_kZr * F_face[j] * n_cross_I_face_conj[i] * JxW_face)
                    .real();
          }
          ...
          for (const auto &[i, j] : B_hat_FE) {
            B_hat_matrix(i, j) -=
                (kz_kZr * E_hat[j] * F_face_conj[i] * JxW_face).real();
          }
      ...
      // (M_4 l - M_2 \hat{u}_h)$ on each cell. When this is obtained, we can
      // perform at the same time the residual estimator (\Psi = G^{-1}(l-B u_h
      // - \hat{B}\hat{u}_h)).
      if (solve_interior) {

        // We first get the solution vector for this cell.
        cell_skeleton->get_dof_values(locally_relevant_solution_skeleton,
        ...
        cell_test->distribute_local_to_global(cell_residual,
                                              locally_owned_residual_estimator);
  ...
  // After the loop over the cells, we finalize the assembly by compressing the
  // vectors because of the MPI parallelization according to the flag
  // solve_interior.
  if (solve_interior) {
...
// has nothing specific to DPG per se. Nonetheless, the method allows us to use
...
template <int dim> void Time_Harmonic_Maxwell_DPG<dim>::solve_skeleton() {
  ...
  pcout << "   " << n_CG_iter << " CG iterations needed to obtain convergence."
        << std::endl;
  if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0) {
...
// faces, we made use of the DataOutFaces class to output it properly. Note that
// the subdomains are only outputed in the interior solution file.
template <int dim>
void Time_Harmonic_Maxwell_DPG<dim>::output_results(const unsigned int cycle) {
  ...
  data_out.add_data_vector(
      locally_relevant_solution_interior, solution_interior_names,
      DataOut<dim>::type_automatic, data_component_interpretation);
  ...
  data_out.write_vtu_with_pvtu_record("./output/", "solution_waveguide", cycle,
                                      mpi_communicator, 3);
  ...
  data_out_test.add_data_vector(locally_relevant_residual_estimator,
                                residuals_names, DataOut<dim>::type_automatic,
                                data_component_interpretation);

  data_out_test.add_data_vector(
      estimated_error_per_cell, std::string("error_norm"),
      DataOut<dim>::type_automatic,
      std::vector<DataComponentInterpretation::DataComponentInterpretation>(
          1, DataComponentInterpretation::component_is_scalar));

  data_out_test.build_patches(fe_test.degree);
  data_out_test.write_vtu_with_pvtu_record("./output/", "residual_waveguide",
                                           cycle, mpi_communicator, 3);
...
// Because we want to have all the error for all the different components of our
// solution vectors separately, we cannot use the
// VectorTools::integrate_difference function directly. Instead, we will perform
// the computation "by hand" by looping over all the cells and faces,
// interpolating the solution at the quadrature points, and computing the error
// with respect to the analytical solution at those points. Since we are
// performing this loop, at the same time we can also compute the error on each
// cell using the energy norm that we defined for the problem. This will be
// useful for the adaptive mesh refinement later on.
template <int dim> void Time_Harmonic_Maxwell_DPG<dim>::calculate_L2_error() {
  ...
  // related to the shape fonctions F and I). These will be used to compute the
  // error in the energy norm on each cell.
  ...
  // the interior to compute its L2 error and the residual of each field on each
  // cell.
  for (const auto &cell : dof_handler_trial_interior.active_cell_iterators()) {
    ...
    if (cell->is_locally_owned()) {
      ...
      for (const unsigned int q_index :
        ...
        // Loop over dimensions to compute each component of the error because
        // for a vector field is simply the sum of the squared scalar L² errors
        // of each component.
        for (unsigned int i = 0; i < dim; ++i) {

          // Calculate the L2 error for E
          L2_error_E_real += pow((local_E_values_real[q_index][i] -
          ...
          L2_error_H_real += pow((local_H_values_real[q_index][i] -
          ...
          // ||F||^2
          complex_shape_function = local_F_values_real[q_index][i] +
          ...
          // ||I||^2
          complex_shape_function = local_I_values_real[q_index][i] +
          ...
          complex_shape_function =
              ...
              conj_ikepsilon_Zr * (local_F_values_real[q_index][i] +
                                   imag * local_F_values_imag[q_index][i]);
          estimated_error_per_cell(cell_index) +=
              (pow(complex_shape_function.real(), 2) +
               pow(complex_shape_function.imag(), 2)) *
      ...
      for (const auto &face : cell->face_iterators()) {
        ...
        fe_face_values_trial_skeleton[extractor_E_real].get_function_values(
            locally_relevant_solution_skeleton, local_E_hat_values_real);
        fe_face_values_trial_skeleton[extractor_E_imag].get_function_values(
            locally_relevant_solution_skeleton, local_E_hat_values_imag);

        fe_face_values_trial_skeleton[extractor_H_real].get_function_values(
            locally_relevant_solution_skeleton, local_H_hat_values_real);
        fe_face_values_trial_skeleton[extractor_H_imag].get_function_values(
            locally_relevant_solution_skeleton, local_H_hat_values_imag);

        // The energy norm only has a face contribution the face where we
        // applied a Robin Boundary condition, i.e., the inlet port and outlet
        // port faces. So we only extract the residual estimator
        // solution on these faces to save some computational time.
        if ((face->boundary_id() == 4) || (face->boundary_id() == 5)) {
          // Extract local solution
          fe_face_values_test[extractor_E_real].get_function_values(
              locally_relevant_residual_estimator, local_F_hat_values_real);
          fe_face_values_test[extractor_E_imag].get_function_values(
              locally_relevant_residual_estimator, local_F_hat_values_imag);

          fe_face_values_test[extractor_H_real].get_function_values(
              locally_relevant_residual_estimator, local_I_hat_values_real);
          fe_face_values_test[extractor_H_imag].get_function_values(
              locally_relevant_residual_estimator, local_I_hat_values_imag);
        }
        ...
        for (const unsigned int &q_index :
          ...
          const double JxW = fe_face_values_trial_skeleton.JxW(q_index);
          const auto &normal =
          ...
          if ((face->boundary_id() == 4) || (face->boundary_id() == 5)) {
            ...
            local_n_I_hat_values_real =
                cross_product_3d(normal, local_I_hat_values_real[q_index]);
            local_n_I_hat_values_imag =
                cross_product_3d(normal, local_I_hat_values_imag[q_index]);
          }
          ...
          for (unsigned int i = 0; i < dim; ++i) {
            ...
            if ((face->boundary_id() == 4) || (face->boundary_id() == 5)) {
              // || n x I + kz_kZr * F||^2
              complex_shape_function =
                  conj_kz_kZr * (local_F_hat_values_real[q_index][i] +
  ...
  pcout << "L2 E real part error is : " << L2_error_E_real_global << std::endl;
  pcout << "L2 E imag part error is : " << L2_error_E_imag_global << std::endl;
  pcout << "L2 H real part error is : " << L2_error_H_real_global << std::endl;
  pcout << "L2 H imag part error is : " << L2_error_H_imag_global << std::endl;
...
// interior.
template <int dim> void Time_Harmonic_Maxwell_DPG<dim>::run() {
  ...
  while (residual_indicator > stopping_criteria) {
    ...
    const double cell_size = Utilities::MPI::max(
        GridTools::maximal_cell_diameter<dim>(triangulation), mpi_communicator);
    ...
    cycle++;
    const double max_residual = Utilities::MPI::max(
        estimated_error_per_cell.linfty_norm(), mpi_communicator);
  ...
  try {
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
      std::cout << "===========================================" << std::endl
                ...
                << "Test order: " << parameters.delta_degree + parameters.degree
                << std::endl
                << "===========================================" << std::endl
                << std::endl;
    }
