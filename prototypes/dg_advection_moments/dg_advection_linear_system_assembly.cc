...
/* *
 * 1D DG Advection Solver (MeshWorker Version - Final Fix)
...
AdvectionDG<dim>::setup_system()
{
  ...
  triangulation.refine_global(6); // 64 cells
  dof_handler.distribute_dofs(fe);

  std::cout << "Domain: [0, 2pi]" << std::endl;
...
template <int dim>
void
AdvectionDG<dim>::assemble_mass_matrix()
{
  mass_matrix = 0;
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int i = 0; i < dofs_per_cell; ++i)
        for (unsigned int j = 0; j < dofs_per_cell; ++j)
          mass_matrix.add(indices[i], indices[j], cell_mass(i, j));
    }
}
...
AdvectionDG<dim>::assemble_rhs()
{
  ...
  // 2. Boundary Worker
  // FIX: Removed 'subface_no' argument. Boundary worker takes 4 args.
      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
          ...
          if (beta_dot_n < 0) // Inflow
            u_flux = this->inflow_function.value(
  ...
  // 3. Face Worker (Internal)
  // Must accept subface args for interface compatibility, but we conditionally
  // use them.
  auto face_worker =
    [&](const typename DoFHandler<dim>::active_cell_iterator &cell,
        const unsigned int                                    face_no,
        ...
        ScratchData<dim>  &scratch,
        CopyData          &copy) {
      // FIX: Use if constexpr to handle 1D case (where 3-arg reinit doesn't
      // exist)
      if constexpr (dim == 1)
        {
          scratch.fe_face_values.reinit(cell, face_no);
...
template <int dim>
void
AdvectionDG<dim>::solve_rhs()
{
  SolverControl        solver_control(1000, 1e-12);
  SolverCG<>           solver(solver_control);
  PreconditionJacobi<> preconditioner;
  preconditioner.initialize(mass_matrix);
  solver.solve(mass_matrix, solution_update, system_rhs, preconditioner);
}
...
AdvectionDG<dim>::run()
{
  ...
  std::cout << "Starting MeshWorker time loop..." << std::endl;
...
AdvectionDG<dim>::output_results(const unsigned int step_number)
{
  ...
  std::string filename =
    "output/solution-" + Utilities::int_to_string(step_number, 5) + ".vtu";
