...
template <int dim> class GLSNavierStokesSolver {
public:
...
private:
  ...
  void assembleGLS(const bool initial_step);
  ...
  void solve_system(bool initial_step, double relative_residual,
                    double minimum_residual); // Interface function

  /**
   * GMRES solver with ILU(N) preconditioning
   */
  void solve_system_GMRES(bool initial_step, double absolute_residual,
                          double relative_residual);

  /**
   * BiCGStab solver with ILU(N) preconditioning
   */
  void solve_system_BiCGStab(bool initial_step, double absolute_residual,
                             double relative_residual);

  /**
   * AMG preconditioner with ILU smoother and coarsener and GMRES final solver
   */
  void solve_system_AMG(bool initial_step, double absolute_residual,
                        double relative_residual);
   ...
   */
  void write_output_results(const std::string folder,
                            const std::string solutionName,
                            const unsigned int cycle, const double time);
...
GLSNavierStokesSolver<dim>::GLSNavierStokesSolver(
    ...
    const unsigned int degreeVelocity, const unsigned int degreePressure)
    : mpi_communicator(MPI_COMM_WORLD),
      ...
      nsparam(p_nsparam), dof_handler(triangulation),
      fe(FE_Q<dim>(degreeVelocity), dim, FE_Q<dim>(degreePressure), 1),
      degreeVelocity_(degreeVelocity), degreePressure_(degreePressure),
...
template <int dim> GLSNavierStokesSolver<dim>::~GLSNavierStokesSolver() {
...
template <int dim> void GLSNavierStokesSolver<dim>::finish_time_step() {
...
template <int dim> void GLSNavierStokesSolver<dim>::set_periodicity() {
  ...
  for (unsigned int i_bc = 0; i_bc < nsparam.boundaryConditions.size; ++i_bc) {
    if (nsparam.boundaryConditions.type[i_bc] == BoundaryConditions::periodic) {
      ...
      GridTools::collect_periodic_faces(
          triangulation, nsparam.boundaryConditions.id[i_bc],
...
template <int dim> void GLSNavierStokesSolver<dim>::setup_dofs() {
  ...
  // Non-zero constraints
  {
    ...
    for (unsigned int i_bc = 0; i_bc < nsparam.boundaryConditions.size;
         ++i_bc) {
      if (nsparam.boundaryConditions.type[i_bc] == BoundaryConditions::noslip) {
        VectorTools::interpolate_boundary_values(
            mapping, dof_handler, nsparam.boundaryConditions.id[i_bc],
            ZeroFunction<dim>(dim + 1), nonzero_constraints,
            fe.component_mask(velocities));
      } else if (nsparam.boundaryConditions.type[i_bc] ==
                 BoundaryConditions::slip) {
        std::set<types::boundary_id> no_normal_flux_boundaries;
        no_normal_flux_boundaries.insert(nsparam.boundaryConditions.id[i_bc]);
        ...
        VectorTools::interpolate_boundary_values(
            ...
            nonzero_constraints, fe.component_mask(velocities));
      }
        ...
        DoFTools::make_periodicity_constraints<DoFHandler<dim>>(
            dof_handler, nsparam.boundaryConditions.id[i_bc],
            nsparam.boundaryConditions.periodic_id[i_bc],
            nsparam.boundaryConditions.periodic_direction[i_bc],
            nonzero_constraints);
      }
...

  {
    ...
    for (unsigned int i_bc = 0; i_bc < nsparam.boundaryConditions.size;
             ...
             // || Parameters::function)
      {
        VectorTools::interpolate_boundary_values(
            mapping, dof_handler, nsparam.boundaryConditions.id[i_bc],
            ZeroFunction<dim>(dim + 1), zero_constraints,
            fe.component_mask(velocities));
      }
    }
  }
  ...
  present_solution.reinit(locally_owned_dofs, locally_relevant_dofs,
                          mpi_communicator);
  solution_m1.reinit(locally_owned_dofs, locally_relevant_dofs,
                     mpi_communicator);
  solution_m2.reinit(locally_owned_dofs, locally_relevant_dofs,
                     mpi_communicator);
  solution_m3.reinit(locally_owned_dofs, locally_relevant_dofs,
                     mpi_communicator);
  ...
  SparsityTools::distribute_sparsity_pattern(
      dsp, dof_handler.n_locally_owned_dofs_per_processor(), mpi_communicator,
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q) {
        ...
        if (scheme == Parameters::SimulationControl::steady)
          ...
          tau = 1. / std::sqrt(std::pow(sdt, 2) + std::pow(2. * u_mag / h, 2) +
                               9 * std::pow(4 * viscosity_ / (h * h), 2));
        ...
        if (assemble_matrix) {
          for (unsigned int j = 0; j < dofs_per_cell; ++j) {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i) {
              local_matrix(i, j) +=
                  (viscosity_ * scalar_product(grad_phi_u[j], grad_phi_u[i]) +
              ...
              if (scheme == Parameters::SimulationControl::bdf1 ||
                  ...
                  scheme == Parameters::SimulationControl::bdf3)
                local_matrix(i, j) +=
                    phi_u[j] * phi_u[i] * alpha_bdf[0] * fe_values.JxW(q);
 ...
 **/
template <int dim>
void GLSNavierStokesSolver<dim>::set_initial_condition(
    Parameters::InitialConditionType initial_condition_type, bool restart) {
  if (restart) {
    pcout << "************************" << std::endl;
    ...
    postprocess();
             ...
             Parameters::InitialConditionType::nodal) {
    set_nodal_values();
    finish_time_step();
    postprocess();
  }
           ...
           Parameters::InitialConditionType::viscous) {
...
void GLSNavierStokesSolver<dim>::iterate(bool firstIteration) {
  ...
  // This is the first iteration
    ...
    if (simulationControl.getMethod() ==
      ...
      simulationControl.setTimeStep(timeParameters.dt *
                                    timeParameters.startup_timestep_scaling);
      ...
      // Reset the time step and do a bdf 3 newton iteration using the two steps
      // to complete the full step
...
template <int dim> void GLSNavierStokesSolver<dim>::assemble_L2_projection() {
  system_matrix = 0;
  system_rhs = 0;
  QGauss<dim> quadrature_formula(degreeQuadrature_);
  const MappingQ<dim> mapping(degreeVelocity_,
...
template <int dim> double GLSNavierStokesSolver<dim>::calculate_CFL() {
...
template <int dim> void GLSNavierStokesSolver<dim>::set_nodal_values() {
  const FEValuesExtractors::Vector velocities(0);
  const FEValuesExtractors::Scalar pressure(dim);
  const MappingQ<dim> mapping(degreeVelocity_,
                              nsparam.femParameters.qmapping_all);
  VectorTools::interpolate(mapping, dof_handler, nsparam.initialCondition->uvwp,
                           newton_update, fe.component_mask(velocities));
  VectorTools::interpolate(mapping, dof_handler, nsparam.initialCondition->uvwp,
                           newton_update, fe.component_mask(pressure));
...
template <int dim>
void GLSNavierStokesSolver<dim>::solve_system(const bool initial_step,
                                              double relative_residual,
...
template <int dim>
void GLSNavierStokesSolver<dim>::solve_system_GMRES(const bool initial_step,
  ...
  if (nsparam.linearSolver.verbosity != Parameters::LinearSolver::quiet) {
  ...
  SolverControl solver_control(nsparam.linearSolver.max_iterations,
                               linear_solver_tolerance, true, true);
...
template <int dim>
void GLSNavierStokesSolver<dim>::solve_system_BiCGStab(
    const bool initial_step, double absolute_residual,
    double relative_residual) {
  TimerOutput::Scope t(computing_timer, "solve");
...
template <int dim>
void GLSNavierStokesSolver<dim>::solve_system_AMG(const bool initial_step,
                                                  double absolute_residual,
                                                  double relative_residual) {
  TimerOutput::Scope t(computing_timer, "solve");
  ...
  TrilinosWrappers::PreconditionAMG::AdditionalData preconditionerOptions(
      ...
      constant_modes, smoother_sweeps, smoother_overlap, output_details,
      smoother_type, coarse_type);
...
template <int dim> void GLSNavierStokesSolver<dim>::refine_mesh() {
...
template <int dim> void GLSNavierStokesSolver<dim>::refine_mesh_Kelly() {
  // Time monitoring
  ...
  if (nsparam.meshAdaptation.variable == Parameters::MeshAdaptation::pressure) {
    KellyErrorEstimator<dim>::estimate(
        mapping, dof_handler, QGauss<dim - 1>(degreeQuadrature_ + 1),
  ...
  if (nsparam.meshAdaptation.fractionType ==
...
template <int dim> void GLSNavierStokesSolver<dim>::refine_mesh_uniform() {
  TimerOutput::Scope t(computing_timer, "refine");
...
void GLSNavierStokesSolver<dim>::newton_iteration(const bool is_initial_step) {
  ...
  bool first_step = is_initial_step;
  {
    ...
    last_res = 1.0;
    current_res = 1.0;
    while ((first_step || (current_res > nsparam.nonLinearSolver.tolerance)) &&
      ...
      if (first_step) {
        ...
        if (nsparam.nonLinearSolver.verbosity != nsparam.nonLinearSolver.quiet)
          pcout << "Initial Newton iteration: " << outer_iteration
                << "  - Residual:  "
                << std::setprecision(nsparam.nonLinearSolver.display_precision)
                << current_res << std::endl;
        solve_system(first_step, nsparam.linearSolver.relative_residual,
        ...
        for (double alpha = 1.0; alpha > 1e-3; alpha *= 0.5) {
          ...
          assemble_rhs(first_step);
          current_res = system_rhs.l2_norm();
          if (nsparam.nonLinearSolver.verbosity !=
              nsparam.nonLinearSolver.quiet)
...
template <int dim> void GLSNavierStokesSolver<dim>::postprocess() {
  if (simulationControl.isOutputIteration())
    write_output_results(
        simulationControl.getOutputFolder(), simulationControl.getOuputName(),
        simulationControl.getIter(), simulationControl.getTime());
...
template <int dim> void GLSNavierStokesSolver<dim>::write_checkpoint() {
...
template <int dim> void GLSNavierStokesSolver<dim>::read_checkpoint() {
  ...
  try {
    ...
    AssertThrow(false, ExcMessage("Cannot open snapshot mesh file or read the "
...
template <int dim>
void GLSNavierStokesSolver<dim>::write_output_results(
    const std::string folder, const std::string solutionName,
    const unsigned int iter, const double time) {
...
template <int dim> void GLSNavierStokesSolver<dim>::write_output_forces() {
  TimerOutput::Scope t(computing_timer, "output_forces");
  for (unsigned int boundary_id = 0;
       boundary_id < nsparam.boundaryConditions.size; ++boundary_id) {
...
template <int dim> void GLSNavierStokesSolver<dim>::write_output_torques() {
...
// sol'n
template <int dim> double GLSNavierStokesSolver<dim>::calculate_L2_error() {
    ...
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int q = 0; q < n_q_points; q++) {
        ...
        if (dim == 3) {
          ...
          l2errorU +=
              (uz_sim - uz_exact) * (uz_sim - uz_exact) * fe_values.JxW(q);
        }
      }
    }
  }
...
template <int dim> double GLSNavierStokesSolver<dim>::calculate_average_KE() {
...
double GLSNavierStokesSolver<dim>::calculate_average_enstrophy() {
  ...
  for (; cell != endc; ++cell) {
    if (cell->is_locally_owned()) {
      ...
      for (unsigned int q = 0; q < n_q_points; q++) {
        //        en +=
        //        0.5*(present_velocity_curl[q]*present_velocity_curl[q])*fe_values.JxW(q)
        //        / globalVolume_;
...
template <int dim> void GLSNavierStokesSolver<dim>::calculate_forces() {
          ...
          if (cell->face(face)->at_boundary()) {
            ...
            if (cell->face(face)->boundary_id() == boundary_id) {
              ...
              for (int q = 0; q < n_q_points; q++) {
                ...
                fluid_stress = viscosity * (velocity_gradients[q] +
  ...
  if (nsparam.forcesParameters.verbosity == Parameters::Forces::verbose &&
      this_mpi_process == 0) {
    std::cout << std::endl;
    TableHandler table;
    ...
    for (unsigned int boundary_id = 0;
      ...
      table.set_precision("f_x", nsparam.forcesParameters.display_precision);
      table.set_precision("f_y", nsparam.forcesParameters.display_precision);
      if (dim == 3) {
        table.add_value("f_z", forces_[boundary_id][2]);
        table.set_precision("f_z", nsparam.forcesParameters.display_precision);
      }
    }
    std::cout << "+------------------------------------------+" << std::endl;
...
template <int dim> void GLSNavierStokesSolver<dim>::calculate_torques() {
  ...
  if (nsparam.forcesParameters.verbosity == nsparam.forcesParameters.verbose &&
    ...
    for (unsigned int boundary_id = 0;
      ...
      table.set_precision("T_x", nsparam.forcesParameters.display_precision);
      table.set_precision("T_y", nsparam.forcesParameters.display_precision);
      table.add_value("T_z", torques_[boundary_id][2]);
      table.set_precision("T_z", nsparam.forcesParameters.display_precision);
    }
 ...
 */
template <int dim> void GLSNavierStokesSolver<dim>::read_mesh() {
  // GMSH input
  ...
  // Primitive input
    ...
    if (nsparam.mesh.primitiveType == Parameters::Mesh::hyper_cube) {
      GridGenerator::hyper_cube(this->triangulation, this->nsparam.mesh.hc_left,
      ...
      GridGenerator::hyper_shell(
          this->triangulation, circleCenter, this->nsparam.mesh.hs_inner_radius,
          this->nsparam.mesh.hs_outer_radius, 4, this->nsparam.mesh.colorize);
...
/*
 * Generic CFD Solver application that is generally appropriate for most CFD
 * problems
 */
template <int dim> void GLSNavierStokesSolver<dim>::solve() {
  this->read_mesh();
  this->setup_dofs();
