...
template <int dim> class NSBoundaryConditions {
public:
...
template <int dim> void NSBoundaryConditions<dim>::createDefaultNoSlip() {
  id.resize(1);
  id[0] = 0;
  type.resize(1);
  type[0] = noslip;
...
template <int dim>
void NSBoundaryConditions<dim>::declareDefaultEntry(ParameterHandler &prm,
                                                    unsigned int i_bc) {
  ...
  prm.declare_entry("id", Utilities::int_to_string(i_bc, 2),
                    Patterns::Integer(), "Mesh id for boundary conditions");

  prm.declare_entry("periodic_id", "0", Patterns::Integer(),
...
template <int dim>
void NSBoundaryConditions<dim>::parse_boundary(ParameterHandler &prm,
                                               unsigned int i_bc) {
...
void NSBoundaryConditions<dim>::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry("number", "0", Patterns::Integer(),
                      "Number of boundary conditions");
    id.resize(max_size);
...
template <int dim> class FunctionDefined : public Function<dim> {
private:
  Functions::ParsedFunction<dim> *u;
  Functions::ParsedFunction<dim> *v;
  Functions::ParsedFunction<dim> *w;
...
public:
  FunctionDefined(Functions::ParsedFunction<dim> *p_u,
      ...
      : Function<dim>(dim + 1), u(p_u), v(p_v), w(p_w) {}
...
template <int dim> class PoiseuilleInlet : public Function<dim> {
public:
...
template <int dim> class ConstantXInlet : public Function<dim> {
public:
  ConstantXInlet() : Function<dim>(dim + 1) { value_ = 1.; };
...
template <int dim> class ConstantXSlip : public Function<dim> {
public:
  ConstantXSlip() : Function<dim>(dim + 1) { value_ = 1.; };
