...
Vector<double> delta(unsigned int p, unsigned int n, unsigned int j, Vector<double> times)
{
  if (j==0)
  {
  ...
  else
  {
    ...
    for (unsigned int i_del = 0 ; i_del < p+1 ; ++i_del)
      delta_sol[i_del] = (delta_1[i_del] - delta_2[i_del])/(times[n]-times[n+j]);
    return delta_sol;
  }
}


Vector<double> bdf_coefficients(unsigned int p, std::vector<double> dt)
{
