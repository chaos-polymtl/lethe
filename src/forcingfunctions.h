...
template<int dim>
class MMS3DSineForcingFunction : public Function<dim>
{
public:
                                                     ...
                                                     Vector<double> &values) const
{
    ...
    const double y = p[1];
    const double z = p[2];
    values(0) = 2*a*a*(-3*cos(2*a*x) + 2.)*sin(a*y)*sin(a*z)*cos(a*y)*cos(a*z);
    values(1) = 2*a*a*(-3*cos(2*a*y) + 2)*sin(a*x)*sin(a*z)*cos(a*x)*cos(a*z);
    values(2) = 4*a*a*(3*cos(2*a*z) - 2)*sin(a*x)*sin(a*y)*cos(a*x)*cos(a*y);

    //Convection terms:
    values(0) += a*(2*pow(cos(a*y),2) - pow(cos(a*z),2))*pow(sin(a*x),3)*pow(sin(a*y),2)*pow(sin(a*z),2)*cos(a*x);
    values(1) += a*(2*pow(cos(a*x),2) - pow(cos(a*z),2))*pow(sin(a*x),2)*pow(sin(a*y),3)*pow(sin(a*z),2)*cos(a*y);
    values(2) += 2*a*(pow(cos(a*x),2) + pow(cos(a*y),2))*pow(sin(a*x),2)*pow(sin(a*y),2)*pow(sin(a*z),3)*cos(a*z);
