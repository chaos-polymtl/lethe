...
As part of the v1.0.0 release, the major components of this version are detailed in the following Journal Article (which is available in open-access):
https://doi.org/10.1016/j.cpc.2025.109880

Lethe v1.1.0 introduces some key new features and bug fixes, including:

- New postprocessing of the melt volume for cases with phase change.

- The version of our Discontinuous Petrov-Galerkin solver for the time-harmonic Maxwell equations is now coupled to the heat transfer.

- New examples for the mortar method named "Paddle Mixer Using the Mortar Method".

- New support for pressure boundary conditions in the matrix-free solvers, including two application tests. Improved the documentation on the pressure boundary conditions.

- Renaming of all occurrences of "VOF", "interface regularization", "algebraic interface reinitialization", and "phase fraction" with "CLS", "interface reinitialization", "pde-based interface reinitialization", and "phase indicator", respectively.

- Refactoring of the way the display precision on the terminal in Lethe is parametrized,  of the h-adaptivity, of the Lethe custom meshes, of  ALE velocity computation and of CFD-DEM solvers (`CFDDEMSolver` and `CFDDEMMatrixFree`) to centralize DEM sub-iteration management.

- Bug fixes of dynamic time-stepping functionality of the unresolved CFD-DEM,  contact between particles and solid objects for DEM, restarts procedure for the lethe-fluid-particles-matrix-free solver and missing factor in the uniform and Gaussian laser models.
