The Lethe v1.0.3 requires the latest major release of the deal.II library (deal.ii v9.7).
...
As of Lethe v1.0.3 the following applications are deprecated and will be removed:
- lethe-fluid-block: The lethe-fluid-block solver was always very experimental. It has not seen any significant development in the last 2-3 years and its structure is now outdated. Although we do not abandon the idea of developing interesting block preconditioner for the stabilized Navier-Stokes formulation, when we do so, we will do it by writing  a new solver from scratch instead of patching the existing one. 
- lethe-rpt-3d, lethe-rpt-cell-reconstruction-3d, lethe-rpt-fem-reconstruction-3d and lethe-rpt-l2-projection-3d. Although they are production ready, the RPT solvers of Lethe have not been actively developed for more than a year. We increasingly felt that they did not belong inside of a CFD software like Lethe, but that they instead should evolve to become their own independent repository. This has already been accomplished and they are now stored on a separate repository: https://github.com/chaos-polymtl/lethe-rpt. Consequently, they will be progressively removed before the next release.

Lethe v1.0.3 introduces some key new features and bug fixes including:

- A new parallel particle-ray tracing solver that can be used to measure the roughness of particle assemblies obtained through DEM simulation. This solver is production ready.
- A new way to manage the coupling in unresolved CFD-DEM simulation. The drag coupling strategy enables the usage of fully explicit, implicit and semi-implicit drag coupling formulation. This feature is part of our effort to migrate the unresolved CFD-DEM solver from a cell-based coupling strategy to a fully filtered formulation.
- A new improved Discontinuous Galerkin (DG) formulation for the tracer auxiliary physics. This feature enables simulations at high Péclet number with minimal artificial diffusion. Although the feature was already present in Lethe, v1.0.3 introduces significant improvement to the formulation when the velocity field is not perfectly divergence-free.
- A new DEM size distribution (log normal) as well as more robust control on particle size distributions.
- Small bug fixes and performance improvements throughout the library.
