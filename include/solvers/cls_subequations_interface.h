...
DeclExceptionMsg(
  ...
  "'set_cls_solution_and_dof_handler' before 'set_cls_solution'.");
...
public:
   ...
   */
  ~CLSSubequationsInterface() = default;
   ...
   */
  void
  setup_specific_subequation_dofs(const CLSSubequationsID &subequation_id)
  {
   ...
   */
  void
  solve_specific_subequation(const CLSSubequationsID &subequation_id)
  {
    AssertThrow((std::find(this->active_subequations.begin(),
                           this->active_subequations.end(),
                           subequation_id) != this->active_subequations.end()),
  ...
  get_dof_handler(const CLSSubequationsID &subequation_id)
  {
    AssertThrow((std::find(this->active_subequations.begin(),
                           this->active_subequations.end(),
                           subequation_id) != this->active_subequations.end()),
  ...
  /**
   * @brief Get a reference to the DoFHandler of the CLS auxiliary physics.
   *
   * @return Reference to the DoFHandler of the CLS auxiliary physics associated
   * to the set CLS solution field.
   */
  const DoFHandler<dim> &
  get_cls_dof_handler()
  {
    return this->dof_handler_cls->get();
  }
  ...
  get_solution(const CLSSubequationsID &subequation_id)
  {
  ...
  get_cls_solution()
  {
    return *this->cls_solution_vector;
  }

  /**
   * @brief Check and return a boolean indicating if the solution of the
   * subequation has been solved for the corresponding @p dof_handler_cls
   * and/or @p cls_solution_vector previously set with
   ...
   * @p dof_handler_cls and/or @p cls_solution_vector.
   *  - Otherwise, @p false.
   */
  bool
  get_solution_validity(const CLSSubequationsID &subequation_id)
  {
    AssertThrow((std::find(this->active_subequations.begin(),
                           this->active_subequations.end(),
                           subequation_id) != this->active_subequations.end()),
   ...
   */
  std::string
  get_subequation_string(const CLSSubequationsID &subequation_id)
  {
    AssertThrow((std::find(this->active_subequations.begin(),
                           this->active_subequations.end(),
                           subequation_id) != this->active_subequations.end()),
    ...
    if (subequation_id == CLSSubequationsID::phase_gradient_projection)
   ...
   */
  void
  set_dof_handler(const CLSSubequationsID         &subequation_id,
   ...
   */
  void
  set_solution(const CLSSubequationsID          &subequation_id,
  ...
  /**
   * @brief Set a new CLS solution vector and DoFHandler for the subequations
   * to be solved and reset their validity.
   *
   * @param[in] cls_solution_vector CLS solution vector associated with the
   * subequations to be solved.
   *
   * @param[in] dof_handler_cls Reference to the DoFHandler associated with the
   * specified CLS solution.
   */
  void
  set_cls_solution_and_dof_handler(const GlobalVectorType &cls_solution_vector,
                                   const DoFHandler<dim>  &dof_handler_cls)
  {
    ...
    this->cls_solution_vector = std::cref(cls_solution_vector);
    this->dof_handler_cls     = std::cref(dof_handler_cls);
  }
   ...
   */
  void
  set_solution_valid(const CLSSubequationsID &subequation_id)
  {
    AssertThrow((std::find(this->active_subequations.begin(),
                           this->active_subequations.end(),
                           subequation_id) != this->active_subequations.end()),
...
private:
  ...
  reset_subequations_solutions_validity()
  {
    for (const CLSSubequationsID &subequation_id : this->active_subequations)
      {
  ...
  /// CLS DoFHandler associated with solved equations
  ...
  std::vector<CLSSubequationsID> active_subequations;

  /** Subequations are stored within a map of shared pointer to ensure proper
   * memory management */
  std::map<CLSSubequationsID, std::shared_ptr<PhysicsSubequationsSolverBase>>
    subequations;

  /// Map of DoFHandler of subequations
  std::map<CLSSubequationsID, std::shared_ptr<DoFHandler<dim>>>
    subequations_dof_handler;

  /// Present solutions map
  std::map<CLSSubequationsID, std::shared_ptr<GlobalVectorType>>
    subequations_solutions;

  /**
   * Booleans indicating if the subequation has been solved with the set CLS
   * solution vector. The CLS solution vector is set with
   * CLSSubequationsInterface<dim>::set_cls_solution_and_dof_handler.
   *
   ...
   *
   * We start by setting a CLS solution and DOFHandler
   * (CLSSubequationsInterface<dim>::set_cls_solution_and_dof_handler),
   ...
   * it remains valid until a new CLS solution and/or DOFHandler are
   * set through
   * CLSSubequationsInterface<dim>::set_cls_solution_and_dof_handler.
   *
   * This validation mechanism allows, to check if:
   * - A new CLS solution has been set;
   ...
   */
  std::map<CLSSubequationsID, bool> subequations_solutions_validity;
};
