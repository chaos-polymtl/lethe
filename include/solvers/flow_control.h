  ...
  const MPI_Comm &                      mpi_communicator)
{
  ...
  if (abs(beta_n - 0) < 1e-6 && step_number > 1)
  {
    if (abs(flow_rate_n) < abs(flow_rate_0))
      adjusted = true;
    else if (abs(flow_rate_1n - flow_rate_n) < abs(flow_rate_n - flow_rate_0)
             && abs(threshold - 1.00125) < 1e-6)
      threshold = 1 + 0.5 * (threshold - 1);
  }
  ...
  else if (step_number == 2)
  {
    ...
    beta_n1 = 0.5 * (flow_rate_n - flow_rate_0) / (area * dt);
           ...
           abs(flow_rate_n) < threshold * abs(flow_rate_0)
           && adjusted == false)
  {
    // If the flow rate between intended flow rate and the threshold,
    // it decreases by itself (no force applied)
    beta_n1 = 0;
  }
  else
  {
