 ...
 *
 * Implementation of the Reactive species equations as an auxiliary physics.
 * Equations solved:
 */

#ifndef lethe_reactive_species_h
#define lethe_reactive_species_h
...
template <int dim>
class ReactiveSpecies : public AuxiliaryPhysics<dim, GlobalVectorType>
{
public:
  ReactiveSpecies(MultiphysicsInterface<dim>      *multiphysics_interface,
   ...
   * NB : not implemented for the reactive physics equations for now.
   *
  ...
  /**
   * @brief Prepares Reactive species to write checkpoint
   */
  void
  write_checkpoint() override;

  /**
   * @brief Allows Reactive species physics to set-up solution vector from checkpoint file;
   */
  void
  read_checkpoint() override;

  /**
   * @brief Returns the dof_handler of the Reactive species physics
   */
...
private:
   ...
   * See the documentation for ReactiveSpeciesScratchData for more
   * information
   *
  ...
  // Core elements for the Reactive species equations variables (Phi and eta)
  ...
  std::vector<std::shared_ptr<ReactiveSpeciesAssemblerBase<dim>>> assemblers;
