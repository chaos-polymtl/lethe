...
public:
        ...
        for (unsigned int j = 0; j < dim; j++)
          {
            ...
            computed_quantities[p] = vorticity_vector[p];
          }
      }
  };
   ...
   * ||\mathbf{S}||^2)\f$ where \f$\mathbf{\omega}\f$ is the vorticity vector
   * and \f$\mathbf{S}\f$ is the symmetric deformation tensor.
   */
  template <int dim>
          ...
          for (unsigned int m = 0; m < dim; m++)
            {
              for (unsigned int n = 0; n < dim; n++)
                {
                  p1 +=
                    (vorticity_vector[p][m][n]) * (vorticity_vector[p][m][n]);
                  r1 += (strain_rate_tensor[p][m][n]) *
                        (strain_rate_tensor[p][m][n]);
                }
            }
  ...
  public:
     ...
     *
     * @param[in] p_omega_x Value of the \f$x\f$ component of the SRF's
     * velocity.
     *
     * @param[in] p_omega_y Value of the \f$y\f$ component of the SRF's
     * velocity.
     *
     * @param[in] p_omega_z Value of the \f$z\f$ component of the SRF's
     * velocity.
     */
      ...
      for (unsigned int q = 0; q < n_quadrature_points; ++q)
        {
          ...
          if (dim == 2)
            velocity =
              velocity +
              omega_z * (-1.) * cross_product_2d(inputs.evaluation_points[q]);
   ...
   * domain.
   *
   * The dynamic viscosity is defined as \f$\mu =
   * \frac{\nu}{\rho_\mathrm{ref}}\f$ where \f$\nu\f$ is the kinematic viscosity
   * and \f$\rho_\mathrm{ref}\f$ is the density of the fluid at reference state.
   *
   * The expression for the kinematic viscosity depends on the rheological
   * model of the fluid. See <a href="https://chaos-polymtl.github.io/lethe/
   * documentation/parameters/cfd/physical_properties.html#rheological-models"
  ...
  public:
     ...
     * evaluation points.
     *
     * @param[in] input_data Independent variable of the postprocessing
     * function.
     *
   ...
   *
   * The expression for the density depends on the density model of the
   * material. See <a
  ...
  public:
     ...
     *
     * @param[in] p_id Either fluid ID or solid ID (as initialized in the
     * physical properties). It is used to name the postprocessed variable
     * vector.
     *
