...
public:
  ...
  /** @brief Extracts the velocity and calculates the volume of the particles
   * in the cell
   *
   * @param pic Iterator that marks the begin and end of the particles in the cell
   */
  ...
  /** @brief Computes the void fraction used for calculating the drag force as the 
   * cell void fraction
   */
   ...
   * 
   * @param q_local Quadrature type object that contains the location of the particles
   * relative the the cell's frame of reference.
   */
  ...
  /** @brief Calculates the properties of the fluid at the locations of the particles. At the moment, 
   * only constant properties within the same fluid are supported. When two fluids are present and VOF
   * is used, the properties are calculated based on the filtered phase fraction interpolated at the 
   * location of the particles. These properties are used in the forces calculations in the vans equations.
   * 
   * @param pic Iterator that marks the begin and end of the particles in the cell.
   */
  ...
  /** @brief Calculates the relative velocity and particle Reynolds number at the location
   * of the particles, which will be used in forces calculations in the vans equations.
   * 
   * @param pic Iterator that marks the begin and end of the particles in the cell.
   */
  ...
  /** @brief Interpolates the filter phase fraction value at the location of the particles.
   * The latter values are used in calculating the density and viscosity of the fluid at the 
   * particles' locations.
   * 
   * @param q_local Quadrature type object that contains the location of the particles
   * relative the the cell's frame of reference.
   * 
   * @param current_filtered_solution The present value of the solution for the filtered phase fraction
   * 
   * @param current_filtered_solution The present value of the solution for the filtered phase fraction
   */
  ...
  template <typename VectorType> 
  void 
   calculate_filtered_vof_at_particle_location(const Quadrature<dim> &q_local,
                                    const VectorType                 &current_filtered_solution,
   ...
   * 
   * @param void_fraction_dof_handler The DoFHandler object for the void fraction
   */
    ...
    const DoFHandler<dim>                       &void_fraction_dof_handler)
  {
    ...
    double total_particle_volume = 0;
    total_particle_volume = calculate_particle_properties ();
    ...
    Quadrature<dim> q_local = calculate_particle_location_quadrature_points (); 
    calculate_fluid_fields_at_particle_location (previous_solution, q_local, dof_handler);

    // Create a quadrature for the void fraction that is based on the particle
    // reference location
    if (this->interpolated_void_fraction)
      {
        calculate_void_fraction_at_particle_location(q_local, void_fraction_dof_handler, void_fraction_solution);
      }
    calculate_fluid_properties_at_particle_location();
    calculate_force_parameters_at_particle_location();
  }
    ...
    const VectorType                      &current_filtered_solution)
  {
    ...
    calculate_filtered_vof_at_particle_location(q_local, current_filtered_solution, dof_handler_vof);
    calculate_fluid_properties_at_particle_location();
    calculate_force_parameters_at_particle_location();
  }
