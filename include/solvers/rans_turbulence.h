...
#ifndef lethe_rans_turbulence_h
#define lethe_rans_turbulence_h
...
DeclException1(
  RANSTurbulenceBoundaryConditionMissing,
  types::boundary_id,
  << "The boundary id: " << arg1
  << " is defined in the triangulation, but not as a boundary condition for the RANS physics. Lethe does not assign a default boundary condition to boundary ids. Every boundary id defined within the triangulation must have a corresponding boundary condition defined in the input file.");


/**
 * @brief Implementation of RANS turbulence as an auxiliary physics. The equations
 * solved vary according to the chosen turbulence model
 *
 */

template <int dim>
class RANSTurbulence : public AuxiliaryPhysics<dim, GlobalVectorType>
{
public:
  /**
   * @brief Constructor of the RANS object.
   *
   ...
   *
   */
  RANSTurbulence(MultiphysicsInterface<dim>      *multiphysics_interface,
                 const SimulationParameters<dim> &p_simulation_parameters,
                 std::shared_ptr<parallel::DistributedTriangulationBase<dim>>
                                                    p_triangulation,
                 std::shared_ptr<SimulationControl> p_simulation_control)
    : AuxiliaryPhysics<dim, GlobalVectorType>(
        p_simulation_parameters.non_linear_solver.at(
          PhysicsID::rans_turbulence))
  ...
  /**
   * @brief Prepare RANS Turbulence to write checkpoint.
   */
  void
  write_checkpoint() override;

  /**
   * @brief Allow RANS Turbulence to set up solution vector from checkpoint file.
   */
  void
   ...
   * rans_turbulence allows a turbulent viscosity model function initial
   * condition over the the domain.
   */
  void
...
private:
   ...
   * See the documentation for RANSTurbulenceScratchData for more
   * information
   *
   ...
   * // TODO complete function for RANS turbulence
   *
   */
  ...
  // Core elements for the RANS turbulent simulation
   ...
   */
  std::shared_ptr<Mapping<dim>> rans_mapping;
  /**
   ...
   */
  std::vector<std::shared_ptr<RANSTurbulenceAssemblerBase<dim>>> assemblers;


  /**
   * @brief Contain turbulent viscosity information used for post-processing
   */
  TableHandler turbulent_viscosity;
};
