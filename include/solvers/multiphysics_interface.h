...
public:
  ...
  /**
   * @brief Request the reference to the vector of solid objects.
   * Used in auxiliary physics that needs to apply a boundary condition on a
   * solid through Nitsche immersed boundary method.
   *
   * @param[in] number_solids Number of solids, the value is used to assert if
   * there are solids.
   *
  ...
  /**
   * @brief Sets the shared pointer to the Reynolds stress of the physics in the
   * multiphysics interface.
   * WIP for an upcoming PR, not yet implemented in the solver.
   *
   * @param[in] physics_id The physics ID of the Reynolds stress solution being
   * set
   *
   * @param solution_vector The shared pointer to the solution vector of the physics
   */
  void
  set_reynolds_stress_solutions(
    const PhysicsID                   physics_id,
    std::shared_ptr<GlobalVectorType> solution_vector)
  {
    AssertThrow((std::find(active_physics.begin(),
                           ...
                           physics_id) != active_physics.end()),
                ExcInternalError());
    physics_time_average_solutions[physics_id] =
      solution_vector; // shouldn't be physics_time_average_solutions
  }
   ...
   * physics in the multiphysics interface
   *
   * @param[in] physics_id The physics ID of the DOF handler
   *
   * @param[in] previous_solutions_vector The shared pointer to the vector of
   * previous block vector solutions
   */
...
private:
  ...
  // solver.
  GlobalVectorType *reynolds_stress_solutions; // unused delete ?
