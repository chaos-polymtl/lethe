...
/**
 * @brief Verify that a file that Lethe must read exists and can be opened, and
 * raise an error stated in the user's terms otherwise.
 *
 * This function is meant to be called right before a file whose path comes
 * from the user, either from the command line or from the parameter file, is
 * opened. It distinguishes a file name that was left empty, a file that does
 * not exist and a file that exists but cannot be read, so that the user is
 * told what to correct instead of facing the error that the reader of the file
 * would raise further down.
 *
 * @param[in] file_name Path of the file, exactly as it was given by the user.
 *
 * @param[in] file_description Role of the file in the simulation. It is used
 * to build the error message and should read naturally after "The", for
 * example "parameter file given to the application" or "gmsh mesh file given
 * by 'subsection mesh' - 'set file name'".
 */
void
               ...
               const unsigned int rotation_axis_direction = 2)
{
  if constexpr (dim == 3)
    {
      const unsigned int coord1 = (rotation_axis_direction + 1) % 3;
      const unsigned int coord2 = (rotation_axis_direction + 2) % 3;
  ...
  if constexpr (dim == 3)
    {
      ...
      point[rotation_axis_direction] =
        center_of_rotation[rotation_axis_direction];
    }
  else
    {
