...
struct Timer {
  ...
  // end
  enum class Type { none, iteration, end };
  Type type;
  static void declare_parameters(ParameterHandler &prm);
  void parse_parameters(ParameterHandler &prm);
};
...
struct NonLinearSolver {
  ...
  enum class SolverType { newton, skip_newton, adaptative_newton };
...
struct LinearSolver {
  // Type of linear solver
  enum class SolverType { gmres, bicgstab, amg };
  SolverType solver;
...
struct Mesh {
  ...
  enum class Type { gmsh, dealii, primitive };
  Type type;

  // Primitive types
  enum class PrimitiveType { hyper_cube, hyper_shell, cylinder };
...
struct MeshAdaptation {
  ...
  enum class Type { none, uniform, kelly } type;

  enum class Variable { velocity, pressure } variable;

  // Decision factor for KELLY refinement (number or fraction)
  enum class FractionType { number, fraction } fractionType;
...
struct VelocitySource {
  // Type of linear solver
  enum class VelocitySourceType { none, srf };
