...
public:
  ...
  value(const std::map<field, double> &fields_value) override
  {
    ...
    std::cout << "phase order value in filter = "
              << phase_order_cahn_hilliard_filtered << std::endl;
    if (std::abs(phase_order_cahn_hilliard_filtered) > 1)
      return 0.0;
    else
      return mobility_cahn_hilliard_constant *
             (1 - phase_order_cahn_hilliard_filtered *
               ...
               std::vector<double> &property_vector) override
  {
    ...
    //          property_vector[i] =
    //            mobility_cahn_hilliard_constant *
    //            (1 - phase_order_cahn_hilliard[i] *
    //            phase_order_cahn_hilliard[i]) * (1 -
    ...
    for (unsigned int i = 0; i < property_vector.size(); ++i)
      {
        // std::cout<<"phase order value in filter =
        // "<<phase_order_cahn_hilliard_filtered[i]<<std::endl;
        if (std::abs(phase_order_cahn_hilliard_filtered[i]) > 1)
          property_vector[i] = 0.0;
        else
          property_vector[i] = mobility_cahn_hilliard_constant *
                               (1 - phase_order_cahn_hilliard_filtered[i] *
                                      phase_order_cahn_hilliard_filtered[i]) *
                               (1 - phase_order_cahn_hilliard_filtered[i] *
                                      phase_order_cahn_hilliard_filtered[i]);
      }
  }
  ...
  jacobian(const std::map<field, double> &fields_value, field /*id*/) override
  {
    ...
    //      return 0.0;
    //    else
    //      return -4 * phase_order_cahn_hilliard *
    //      mobility_cahn_hilliard_constant *
    //             (1 - phase_order_cahn_hilliard * phase_order_cahn_hilliard);
    ...
    if (std::abs(phase_order_cahn_hilliard_filtered) > 1)
      ...
      return -4 * phase_order_cahn_hilliard_filtered *
             mobility_cahn_hilliard_constant *
             (1 - phase_order_cahn_hilliard_filtered *
                    phase_order_cahn_hilliard_filtered);
  }
                  ...
                  std::vector<double> &jacobian_vector) override
  {
    ...
    //      else
    //        jacobian_vector[i] =
    //          -mobility_cahn_hilliard_constant * 4 *
    //          phase_order_cahn_hilliard[i] * (1 - phase_order_cahn_hilliard[i]
    ...
    for (unsigned int i = 0; i < jacobian_vector.size(); ++i)
      if (std::abs(phase_order_cahn_hilliard_filtered[i]) > 1)
        ...
        jacobian_vector[i] = -mobility_cahn_hilliard_constant * 4 *
                             phase_order_cahn_hilliard_filtered[i] *
                             (1 - phase_order_cahn_hilliard_filtered[i] *
                                    phase_order_cahn_hilliard_filtered[i]);
  }
