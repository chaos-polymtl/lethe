...
#include <dem/particle_wall_rolling_resistance_torque_models.h>
                             ...
                             const double                   dt)
{
  ...
  // time-step This variable is the main reason that we have iteration over two
  // different vectors (pairs_in_contact and contact_pair_candidates):
 ...
 * information for calculation of the contact force for a particle-wall pair in
 * contact.
                  ...
                  Tensor<1, 3>                    &rolling_resistance_torque)
{
  ...
  if constexpr (contact_model == ParticleWallContactForceModel::JRK)
    {
...
calculate_linear_contact(particle_wall_contact_info<dim> &contact_info,
                         ...
                         Tensor<1, 3>                    &normal_force,
                         Tensor<1, 3>                    &tangential_force,
                         Tensor<1, 3> &particle_tangential_torque,
                         Tensor<1, 3> &rolling_resistance_torque)
{
  // i is the particle, j is the wall.
  ...
  const double particle_radius = particle_properties[PropertiesIndex::dp] * 0.5;
  ...
  const double rolling_viscous_damping_coeff =
    this->effective_rolling_viscous_damping_coefficient[particle_type];
 ...
 */
inline void
calculate_nonlinear_contact(particle_wall_contact_info<dim> &contact_info,
                            const double                     dt,
                            const ArrayView<const double> &particle_properties,
                            Tensor<1, 3>                  &normal_force,
                            Tensor<1, 3>                  &tangential_force,
                            ...
                            Tensor<1, 3> &rolling_resistance_torque)
{
  ...
  // These values are used to consider non-linear relation of the contact force
  // to the normal overlap
                      ...
                      Tensor<1, 3> &rolling_resistance_torque)
{
  ...
  // convention (i -> j)
  Tensor<1, 3> normal_vector   = -contact_info.normal_vector;
  const double particle_radius = 0.5 * particle_properties[PropertiesIndex::dp];
  const unsigned int particle_type = particle_properties[PropertiesIndex::type];
                      ...
                      Tensor<1, 3> &rolling_resistance_torque)
{
  ...
  // No contact, but still in the constant zone for the cohesive force.
  else if (contact_info.normal_overlap > delta_0)
    {
      cohesive_term = -F_po;
      contact_info.tangential_displacement
        .clear(); // clear works or needed to put each term to 0 like done
                  // before?
  ...
  // No contact. The cohesive force is not constant. It needs to be computed.
  else
    {
      ...
      contact_info.rolling_resistance_spring_torque
        .clear(); // idem, check that clear works here instead of setting to 0
    }
