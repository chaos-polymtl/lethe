...
private:
    ...
    Tensor<1, 3>                        &rolling_resistance_torque)
  {
    ...
    const unsigned int pair_index =
      effective_properties->vec_particle_type_index(particle_one_type,
                                                    particle_two_type);

    const double youngs_modulus =
      effective_properties->effective_youngs_modulus[pair_index];
    const double beta = effective_properties->model_parameter_beta[pair_index];
    const double friction_coeff =
      effective_properties->effective_coefficient_of_friction[pair_index];
    const double rolling_friction_coeff =
      effective_properties
        ->ffective_coefficient_of_rolling_friction[pair_index];
