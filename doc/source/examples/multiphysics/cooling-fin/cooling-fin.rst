...
A cylindrical fin of length :math:`L=0.2` and radius :math:`R=0.01` is fixed to a wall at a constant temperature :math:`T_b`. The radial surface of the fin is subject to natural convection following Newton's law of cooling with :math:`h=10` and :math:`T_{\infty}=20`. The material from which is made the fin has a thermal conductivity :math:`k=100\mathrm{W/m/K}`. The geometry of this example is illustrated below. 
...
Boundary conditions must be set for both fluid dynamics and heat tranfer, even though the former is not used. 
