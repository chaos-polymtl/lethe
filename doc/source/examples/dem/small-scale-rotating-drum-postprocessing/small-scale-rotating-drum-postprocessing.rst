...
- ``reference_time_step``: Reference time step to which the modification will be applied. If ``time_dependent = False``, the result of the modification applied to ``reference_time_step`` will be simply replicated to the others. By default: ``0``
...
.. Note:: 
  If we applied the second condition first, since conditions would overlap, instead of 3 layers of particles we would have only two.
...
This will open one interactive window such as this one:
...
To get the index and the position of the nearest neighbor (`0`) of particle `2` at the 5th time step, the following can be used:
...
This method calculates the mixing index for each particles and stores it in an array named according to the parameter `mixing_index_array_name`. This array can be used for visualization.
