...
.. important::
    ...
    The special ``n_procs`` parameter controls the number of cores used in the post-processing routine. By default, its set to ``None``. If no other value is provided, the ``n_procs`` will be the number of CPUs on your machines.
