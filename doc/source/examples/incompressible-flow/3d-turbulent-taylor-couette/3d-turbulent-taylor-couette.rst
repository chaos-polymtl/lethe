...
Indicating an ``initial refinement = 4`` implies that the initial mesh is refined 4 times, i.e., in 3D, each cell is divided by 8 per refinement. However, in addition to this global refinement, the cells next to the walls, are refined one more time locally indicating ``initial boundary refinement = 1`` and the ids for the walls to be refined as follows ``set boundaries refined = 0, 1``. Consequently,  The following figure illustrates the mesh: 
...
.. note::
  ...
  The mesh resolution used in the case study consists of 153,600 cells. WIf one more initial refinement is used, 942,080 cells are obtained. 
...
We then revert the scheme order back to Q2 and refine the mesh by setting the ``initial refinement = 5`` in the mesh subsection resulting in a total of 942,080 cells. While this simulation closely matches the references, we observe that the second peak of enstrophy seems to require higher-order elements to be fully captured:
