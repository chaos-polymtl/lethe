...
Launching of an application requires an executable of the required solver, and a parameters file (with extension .prm). The parameter file controls all aspect of Lethe and drives the simulation. This section aims at describing the various parameters available within Lethe.
...
In the parameter file format, the parameters are established one by one using the following syntax: ``set time end = 10`` would fix the end time to 10. The arguments can be either doubles, integers, strings or a choice between a predefined number of variable. In the parameter files, comments are preceded by the sharp symbol (e.g. '#comment'). The parameter file format has sanity checking mechanism in places and will throw an error if an unknown parameter or an invalid value are entered.
