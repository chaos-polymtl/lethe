...
.. note::
 By default, post-processing options are set to ``false`` to reduce te number of files generated.

--------------------------
Lagrangian post-processing
--------------------------
The ``Lagrangian post-processing`` enables VTU files output generated with the same ``output frequency`` as the particles' results (see the :doc:`../dem/simulation_control` section of this guide). The name of the generated files are set by the ``output name`` parameter in the simulation_control section plus the suffix ``-postprocess_data``. The files include:
...
----------------------------
The ``force chains`` feature enables VTU, PTVU, and PVD files to visualize force chains between particles. The name of the generated files are set by the ``output name`` parameter in the simulation_control section plus the suffix ``-force_chains``. The files include:
...
The following video shows force chains for the `Rectangular Hopper example <https://chaos-polymtl.github.io/lethe/documentation/examples/dem/rectangular-hopper/rectangular-hopper.html>`_.
