...
* ``dynamic disabling contacts`` subsection deals with the enabling of the feature that disables the particle contacts computation when particles in a cell have low motion according to their granular temperature.
  ``enable dynamic disabling contacts`` enabled the feature, ``granular temperature threshold`` is the threshold of the granular temperature below which the contacts are disabled, and ``solid fraction threshold`` is the minimum solid fraction of the cell in which the contacts may be disabled.
