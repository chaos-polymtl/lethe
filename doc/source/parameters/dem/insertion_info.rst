...
Volume
-------
The volume insertion methe uses an insertion box where particles will be insert. the insertion locations of particles are randomly selected if the ``random number range`` is not equal to zero, otherwise, the particles will be perfecly aligns with respect to the x, y and z directions.
