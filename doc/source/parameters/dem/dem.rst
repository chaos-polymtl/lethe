...
The ``mesh`` section defines the simulation triangulation and refinements. Since the DEM and the CFD solvers read their triangulation through the same routine, the ``manifolds`` and ``box refinement`` sections documented for the CFD solvers are also available in ``lethe-particles``.
