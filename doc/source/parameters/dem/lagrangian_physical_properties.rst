...
* For a ``custom`` distribution, the particle size sampling is performed following a user-defined probability density function (PDF) or cumulative density function (CDF) which can be number- or volume-weighted depending on the ``distribution weighting basis`` parameter. The density function used for sampling is defined by the user through a list of diameter values (``custom distribution diameters values``) and a corresponding list of probability values (``custom distribution diameters probabilities``). Linear interpolation between defined diameter values can be used by setting the ``custom distribution interpolation`` parameter to ``true``. If set to ``false``, sampling is performed by respecting the associated CDF of the distribution (even when a PDF is defined by the user), and only the diameters values defined in the list will be present in the simulation.
...
* For ``normal``,  ``lognormal`` or a ``custom`` distributions, the ``distribution prn seed`` parameter defines the pseudo-random number (PRN) generator with which the diameters values are getting generated. When running a simulation in parallel, each MPI process will have its own PRN in increments of one starting from the defined seed. This means that launching the same simulation with a different number of MPI processes will lead to a different initial particle configurations. The increment is done to ensure that no noticeable and repeatable patterns appear in the initial particle configuration when running in parallel.

* The ``minimum diameter cutoff`` and ``maximum diameter cutoff`` parameters define the lower and upper bounds of the diameter sampled from the distribution. If set to ``-1``, those two bounds are set to :math:`\mu_d \pm3 \sigma_d` for a ``normal`` distribution, to :math:`\exp(\mu \pm 3 \sigma)` for a ``lognormal`` distribution and to the lowest and highest values defined in the ``custom distribution diameters values`` list for a ``custom`` distribution. To define the user defined distribution, diameter values are first generated, than if the values is outside of the defined bounds, a new diameter value is generated until it falls within the defined bounds.
