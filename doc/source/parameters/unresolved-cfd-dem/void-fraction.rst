...
* The ``qcm filter type`` selects the kernel used to weigh particle contributions onto the mesh in the ``qcm`` method. Two options are available: ``spherical`` (default) and ``gaussian``. In both cases the characteristic filter length :math:`\ell` is set to half of ``qcm smoothing length`` (i.e. :math:`\ell = d_{\text{smooth}}/2`). For each particle of radius :math:`r_p` and volume :math:`V_p`, the kernel value at distance :math:`d` from the particle centre to a quadrature point is evaluated and then normalised by the sum of all kernel values over the neighbour-cell stencil so that the total particle volume is conserved.

  * **Spherical** (``spherical``): the kernel is the intersection volume between the particle sphere (radius :math:`r_p`) and an averaging sphere of radius :math:`\ell` centred at the quadrature point. In 3-D, for an overlap distance :math:`0 < d < r_p + \ell`, this is
    ...
    and zero when :math:`d \geq r_p + \ell` or :math:`V_p` when :math:`d \leq \ell - r_p`. The characteristic length :math:`\ell` is the averaging-sphere radius.

  * **Gaussian** (``gaussian``): the kernel is the :math:`d`-dimensional isotropic Gaussian probability density scaled by the particle volume,

    .. math::

       K_{\text{gauss}}(r_p,\sigma,d) = V_p \cdot \frac{1}{(2\pi\sigma^2)^{d/2}}
         \exp\!\left(-\frac{d^2}{2\sigma^2}\right)

    where :math:`d` is the spatial dimension and the characteristic length :math:`\ell = \sigma` is the standard deviation. The Gaussian has non-compact support: contributions outside the QCM neighbour-cell stencil are silently dropped, but the discrete renormalisation absorbs the missing mass to preserve conservation. :math:`\sigma` should be chosen small relative to the neighbour-cell stencil reach to avoid bias from the truncation.
