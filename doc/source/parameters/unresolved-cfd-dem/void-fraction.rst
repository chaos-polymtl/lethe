...
* The ``qcm filter type`` selects the kernel used to weigh particle contributions onto the mesh in the ``qcm`` method. Two options are available: ``spherical`` (default) and ``gaussian``. The ``spherical`` filter is the original spherical indicator (Heaviside) kernel and uses the QCM characteristic length as the averaging-sphere radius. The ``gaussian`` filter uses the same characteristic length as the standard deviation :math:`\sigma` of a Gaussian kernel. Note that the Gaussian has non-compact support: contributions outside the QCM neighbor-cell stencil are silently dropped, but the discrete renormalisation absorbs the missing mass to preserve conservation. :math:`\sigma` should be chosen small enough relative to the neighbor-cell stencil reach so that the projected shape is not biased by the truncation.
