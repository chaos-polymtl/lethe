...
* ``subsection interface sharpening``: defines parameters to counter numerical diffusion of the VOF method and to avoid the interface between the two fluids becoming more and more blurry after each time step. The reader is refered to the Interface sharpening section of :doc:`../../../theory/fluid_dynamics/vof` theory guide for additional details on this sharpening method.
...
* ``subsection phase filtration``: This subsection defines the filter applied to the phase fraction. This affects the definition of the interface.
  ...
  * ``type``: defines the filter type, either ``none`` or ``tanh``
    ...
    * ``set type = tanh``: the filter function described in the Interface filtration section of :doc:`../../../theory/fluid_dynamics/vof` theory guide is applied.
