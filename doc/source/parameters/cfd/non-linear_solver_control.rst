...
* The ``solver`` parameter enables to choose the nonlinear solver used. Currently, Lethe supports three non-linear solvers:
	...
	* ``linear`` The system is assembled and solve assuming it is linear (i.e., without constructing a Jacobian matrix). This option is not applicable to the fluid problem.
