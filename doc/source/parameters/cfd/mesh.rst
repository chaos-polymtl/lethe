...
* ``remove manifold after initialization``. Indicates if the manifold should be removed (reset to flat) after initial refinements. This option is useful when having a constant domain boundary is important, for example when flow in a adaptively refined pipe is considered. At the moment, this options is only supported for ``dealii`` grids with ``simplex = false``.
