...
* The following choices for the mesh ``type`` are available:
    ...
    * ``lethe``: if this type is chosen, the grid will be generated by Lethe built-in mesh. These custom meshes are generally used for beanhcmark and are constructed using deal.II meshing tools. The type of grid is specified by the ``grid type`` parameter and the arguments used for grid generation by the ``grid arguments`` parameter.

* The ``lethe`` built-in mesh types that can be generated are the following when defined by the ``grid type`` parameter:
