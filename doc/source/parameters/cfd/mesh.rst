...
* The following ``lethe`` built-in mesh types, defined by the ``grid type`` parameter, can be generated:
    ...
    * ``uniform_channel_with_meshed_square_prism``: This mesh is similar to the ``uniform_channel_with_meshed_cylinder`` but uses a square prism, that can be rotated instead of a cylindrical obstacle.  More details on this type of mesh and its grid arguments will be provided in a future example.
