...
Homogeneous constraints are applied on velocity degrees of freedom (DoFs) of cells found within the prescribed temperature range to mimic a solid.
...
* The ``number of constraints`` parameter is an integer representing the number of constraints that will be applied. It is used in multiphase (CLS) simulations to apply different constraints to each fluid. Only one constraint per fluid can be imposed. Each constraint comes with its own subsection (starting with number ``0``) containing its own set of parameters as detailed below.

  * The ``fluid id`` parameter is an integer representing the fluid on which the current constraint should be applied.

  * The ``phase indicator tolerance`` parameter is an absolute tolerance on the filtered phase indicator :math:`(\phi')` used in conjunction with CLS simulations (see :doc:`conservative_level_set` subsection) to select the cells on which the constraint is applied. For example, if a ``phase indicator tolerance`` of :math:`10^{-4}` is specified for a constraint on a fluid of ``fluid id = 1``, cells with :math:`\phi' \in [0.9999,1.0001]` are considered as the cells of interest. Note that if the phase indicator filter is disabled, the tolerance will be applied on the solved phase indicator field :math:`(\phi)`.
