...
In this method, the two fluids considered are given an index of :math:`0` and :math:`1` respectively. The amount of fluid at any given quadrature point is represented by a phase order parameter :math:`\phi` between :math:`-1` and :math:`1`. The interface between the two fluids is naturally diffuse and the movements of the interface are driven by differences in the local chemical potential :math:`\eta`.
...
- :math:`\mathbf{a}` corresponds to the velocity field; this parameter is used when the problem is driven by convection.
...
* ``mobility model``: defines the model chosen for mobility. The choices are either ``constant`` (default) or ``quartic``.
