...
def vertical_spacing_y(particle):
    ...
    Returns:
    float: Vertical spacing in the y-direction.
    """
...
def standard_deviation(profile_stats):
    ...
    Returns:
    float: Standard deviation or NaN if count is less than 2.
    """
...
def welford(H, grouped_by_x, stats):
    ...
    Returns:
    None: Updates the stats dictionary in place.
    """
...
def plot_data(keys, index, simulation_data, paper_data, axs, color_map, custom_lines):
    ...
    index (int): Index to select the specific set of keys to plot.
