...
legend_handles_sorted = [handles[i] for i in sorted_indices]

lines = [lines[i] for i in sorted_indices]

for line, color, marker in zip(lines, colors, markers):
    line.set_color(color)
    line.set_marker(marker)
    line.set_label(legend_labels)
...
plt.legend(legend_handles_sorted, legend_labels_sorted, prop=font_properties , loc='lower right')  # Adjust the font size and markerscale
...
# We do this because the files are not read in increasing order of degu and then degp
# Combine degu and degp into a list of tuples (degu, degp)
my_list = list(zip(v_degu[v_degu!=3], v_degp[v_degu!=3]))
...
# Define the font properties explicitly
font_properties = font_manager.FontProperties(size=10)  # Adjust size as needed
# Reusing the same legend handles and labels since the same ordering is used for the colors and the markers and the number of polynomials tested is smaller
plt.savefig("Order of convergence_velocity.png", dpi=300, bbox_inches='tight')
# plt.show()
plt.clf() 
...
for line, color, marker in zip(lines, colors, markers):
    ...
    line.set_linestyle('--') 
# Reusing the same legend handles and labels   
plt.savefig("Order of convergence_pressure.png", dpi=300, bbox_inches='tight')
# plt.show()
plt.clf() 
