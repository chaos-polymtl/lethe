...
for ref in mesh_ref:
  for poly_deg_u in poly_deg:
      for poly_deg_p in  range(1, int(poly_deg_u)+1):
          ...
          # Insert the mesh refinement level, the degree of the velocity solution and the degree of the pressure solution in the prm template with Jinja2 and render it
          parameters = template.render(LEVEL = int(ref), Poly_deg_u = int(poly_deg_u), Poly_deg_p = int(poly_deg_p))
