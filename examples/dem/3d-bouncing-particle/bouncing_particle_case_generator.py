...
## This small code provides the Young's modulus of the particle to obtain a desired value of normal stiffness constant.
## To run this code, run this command in your terminal : python3 bouncing_particle_case_generator.py 5000000
...
def Young_effective(Poisson_wall, Young_wall, Poisson_particle, Young_particle):
    ...
    Y_effective = ((1 - Poisson_wall ** 2) / Young_wall + (
                1 - Poisson_particle ** 2) / Young_particle) ** (-1)
    return Y_effective


def f(x):
    y_eff = Young_effective(Wall_prop.poisson_coef_wall, Wall_prop.young_wall,
                            Particle_prop.poisson_coef_particle, x)
    parentheses = (15 * Particle_prop.mass_particle * 1. ** 2. / (
                16 * Particle_prop.radius_particle ** 0.5 * y_eff)) ** 0.2
    return (
                16 / 15) * Particle_prop.radius_particle ** 0.5 * y_eff * parentheses - Param.kn_wanted
...
yp = optimize.bisect(f, Param.y_estim_1,
                     Param.y_estim_2)  # Young's modulus of the particle
print(yp)
...
plt.grid()
plt.title(
    "Normal stiffness constant as a function\n of the Young's modulus of the particle")
