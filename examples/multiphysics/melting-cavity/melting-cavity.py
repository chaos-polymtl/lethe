...
#Load melted volume
t_list_alge,melt_volume_alge=np.loadtxt(output_path+"/melt_volume_alge.dat",unpack=True,skiprows=1)
t_list_geo,melt_volume_geo,_=np.loadtxt(output_path+"/melt_volume_geo.dat",unpack=True,skiprows=1)

melt_frac_alge = [V/total_volume for V in melt_volume_alge]
melt_frac_geo = [V/total_volume for V in melt_volume_geo]

#Plot melted volume
fig1 = plt.figure()
ax1 = fig1.add_subplot(111)
ax1.plot(t_list_alge * time_scaling , melt_frac_alge, '-k', label=r"Simulation - $V_\text{melt,alge}$")
ax1.plot(t_list_geo * time_scaling , melt_frac_geo, '--k', label=r"Simulation - $V_\text{melt,geo}$")
