#############################################################################
"""
Postprocessing code for rising_bubble_VOF example
...
data = pd.read_csv("Output/rising_bubble_VOF.pvd",sep='"',header=5, usecols=[1]) 
data.columns = ["a"] 
time_list = data['a']
print(data['a'])
...
for i in range(0, len(list_vtu)):
    ...
    points = pd.DataFrame(df.points[:, 1])
    points_x = pd.DataFrame(df.points[:, 0])
    phase  = pd.DataFrame(df['phase'])
    
    y_max = max(points[phase[0] > phase_limit].values)[0]
    y_min = min(points[phase[0] > phase_limit].values)[0]
...
#Smooth average y
smoothed_y = savgol_filter(y_list, 7, 2)

#Calculate bubble rise velocity = dy/dt
y_diff_list = np.diff(smoothed_y)
t_diff_list = np.diff(time_list)
bubble_rise_vel = y_diff_list / t_diff_list
bubble_rise_t = time_list[0:-1] + t_diff_list

#Data from Zahedi, Kronbichlet and kreiss (2012)
...
ax0.plot(x_ref, y_ref, 'ro',label="Reference - Zahedi, Kronbichlet and kreiss (2012)")
ax0.set_ylabel(r'Bubble center height')
ax0.set_xlabel(r'$t$')
ax0.legend(loc="upper left")
fig0.savefig(f'./ymean_t.png')
plt.show()

smoothed_bubble_rise_vel = savgol_filter(bubble_rise_vel, 7, 2)

fig1 = plt.figure()
ax1 = fig1.add_subplot(111)
ax1.plot(bubble_rise_t, bubble_rise_vel , '-ok', label="Simulation")
ax1.plot(x_vel, y_vel, 'ro',label="Reference - Zahedi, Kronbichlet and kreiss (2012)")
ax1.set_ylabel(r'Rise velocity')
ax1.set_xlabel(r'$t$')
ax1.legend(loc="upper left")
ax1.legend(loc=4)
fig1.savefig(f'./bubble_rise_velocity.png')
plt.show()
