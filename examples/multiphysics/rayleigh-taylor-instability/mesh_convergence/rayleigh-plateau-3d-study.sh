...
###############################################################################################
# Bash script for generating Rayleigh-Plateau 3d cases and launching them
#
# Run with: ./rayleigh-plateau-3d-study.sh <path_to_template_parameter_file>
#           <path_to_template_job_file> <path_to_cases_parameters_csv_file>
#           <"{sequence_of_case_numbers}"> <-rd>
###############################################################################################
...
USAGE="Usage: $0 [-t <path_to_template_parameter_file>] \n
            ...
            \t[-rd <true/false>] \n
            \n
             -t: Path to template parameter file (default: ./rayleigh-plateau-3d-geo.tpl) \n
             -j: Path to template job file for launching simulations on clusters (default: ./launch.tpl) \n
             -p: Path to CSV file with case parameters (default: ./rayleigh-plateau-3d-cases.csv) \n
...
template_parameter_file="${template_parameter_file:-./rayleigh-plateau-3d.tpl}"
template_job_file="${template_job_file:-./launch.tpl}"
cases_csv_file="${cases_csv_file:-./rayleigh-plateau-3d-cases.csv}"
...
# Function to expand ranges specified in the <"{sequence_of_case_numbers}"> argument
expand_case_numbers() {
  local input="$1"
  local result=()
...
do
    ...
    # Compute initial and minimum refinement levels
    initial_refinement=$((maximum_refinement - 3))
    minimum_refinement=$initial_refinement
    box_refinement=$((maximum_refinement-initial_refinement))
  ...
  echo "  INITIAL_REFINEMENT:       $initial_refinement"
  ...
  sed -i "s/INITIAL_REFINEMENT/$initial_refinement/g" "$case_name/$case_parameter_file"
  sed -i "s/MIN_REFINEMENT/$minimum_refinement/g" "$case_name/$case_parameter_file"
