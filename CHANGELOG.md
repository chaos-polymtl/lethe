...
- MAJOR Added a new feature in the DEM solver. The base weight of a cell can now be define by a parsing function in the parameter file. This allows for cells to have a personalized weight depending on the position of their barycenter in space and time. This features will be useful when dealing with problems requiring few load balancing steps, like the granuheap example, where the tangential overlap is important and needs to be fully tracked. [#1446](https://github.com/chaos-polymtl/lethe/pull/1446)
