...
- MINOR The previous implementation to calculate cell diameter used the `measure` function which assumed a linear mapping. A function that calculates the volume by integrating 1 over the cell, by summing JxW values returned by the FEValues object now replaces the `measure` function. [#1279](https://github.com/chaos-polymtl/lethe/pull/1279)
