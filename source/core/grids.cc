...
{
  // Meshes generated by deal.II (type = dealii) or by Lethe's built-in
  // generators (type = lethe) carry default (deal.II) curved manifolds attached
  // to the triangulation. GridTools::scale moves the vertices but does not scale
  // these attached manifolds, so a scale factor other than 1 leaves the
  // manifolds inconsistent with the mesh and distorts the geometry under
  // refinement. Scaling is therefore only supported for GMSH meshes; dealii and
  // lethe grids must instead be dimensioned through the grid arguments.
  AssertThrow(
    mesh_parameters.type == Parameters::Mesh::Type::gmsh ||
      mesh_parameters.scale == 1.0,
    ExcMessage(
      "Mesh scaling (the mesh 'scale' parameter) is only supported for GMSH "
      "meshes. The 'dealii' and 'lethe' grids carry attached manifolds that "
      "GridTools::scale does not scale, so scaling would distort the geometry "
      "under refinement. Keep 'scale = 1' and set the mesh dimensions through "
      "the grid arguments instead."));
