...

{
  ...
  // generators (type = lethe) may carry curved manifolds attached to the
  // triangulation. GridTools::scale moves the vertices but does not scale these
  // attached manifolds, so a scale factor other than 1 can leave the manifolds
  // inconsistent with the mesh and distort curved geometries under refinement.
  // Scaling is still applied (some meshes have no manifold, or a manifold that
  // is invariant under scaling), but we warn the user so the result can be
  // checked; such geometries can alternatively be dimensioned through the grid
  // arguments.
  if (mesh_parameters.scale != 1.0 &&
      mesh_parameters.type != Parameters::Mesh::Type::gmsh)
    {
      const ConditionalOStream pcout(std::cout,
