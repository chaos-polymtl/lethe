...
namespace Parameters::Lagrangian
{
  ...
  InsertionInfo::parse_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("insertion info");
    {
      ...
      std::vector<std::string> x_str_list(Utilities::split_string_list(x_str));
      std::vector<std::string> y_str_list(Utilities::split_string_list(y_str));
      std::vector<std::string> z_str_list(Utilities::split_string_list(z_str));
      ...
      std::vector<std::string> d_str_list(Utilities::split_string_list(d_str));
  ...
  ModelParameters::declare_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("model parameters");
    {
      ...
      prm.declare_entry("integration method",
                        "velocity_verlet",
                        Patterns::Selection(
                          "velocity_verlet|explicit_euler|gear3"),
  ...
  FloatingWalls<dim>::declare_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("floating walls");
    {
      ...
      for (unsigned int counter = 0; counter < max_number_floating_walls;
           ++counter)
        {
          prm.enter_subsection("wall " + Utilities::int_to_string(counter, 1));
          {
            declareDefaultEntry(prm);
          }
          prm.leave_subsection();
