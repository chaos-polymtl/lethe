...
GridCylinder<dim, spacedim>::GridCylinder(const std::string &grid_arguments)
{
  ...
  if (arguments.size() != 4)
    {
      AssertThrow(
        false,
        ExcMessage(
          "Mandatory cylinder parameters are (plate file name : cylinder file name)"));
    }
  else
    {
      ...
      std::cout << "Plate: " << this->plate_file_name << std::endl;
      std::cout << "Cylinder: " << this->cylinder_file_name << std::endl;
    }
}
...
GridCylinder<3, 3>::make_grid(Triangulation<3, 3> &triangulation)
{
  ...
  std::ifstream plate_input_file(this->cylinder_file_name);
  grid_plate.read_msh(cylinder_input_file);

  // Merge triangulations
  GridGenerator::merge_triangulations(
    tria_plate, tria_cylinder, triangulation, 0.0, true, true);

  // Set cylindrical manifolds
  triangulation.set_manifold(this->cylindrical_bid_plate,
                             CylindricalManifold(Tensor<1, 3>({0, 0, 1}),
                                                 Point<3>({0, 0, 0})));
