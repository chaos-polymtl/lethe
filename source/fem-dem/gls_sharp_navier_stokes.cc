      ...
      if (cell->is_locally_owned() || cell->is_ghost())
        {
          ...
          for (unsigned int p = 0; p < particles.size(); ++p)
            {
              // Check if a cell is cut and if it's return the particle by which
              // it's cut and the local DOFs index. The check is done by
              // counting the number of DOFs that is on either side of the
              // boundary define by a particle.
              ...
              // This mean that the cell is cut so we return that information
              // and the index of the particle that cut the cell as well as the
              // container containing local DOF of the cell.

              if (nb_dof_inside != 0)
                std::tie(cell_is_cut, p, local_dof_indices) =
                  std::make_tuple(true, p, local_dof_indices);
              else
                std::tie(cell_is_cut, p, local_dof_indices) =
                  std::make_tuple(false, 0, local_dof_indices);
