  ...
  for (Triangulation<3>::active_cell_iterator cell = tr.begin_active();
    ...
    for (unsigned int v = 0; v < GeometryInfo<3>::vertices_per_cell; ++v) {
      for (const auto &neighbor : v_to_c[cell->vertex_index(v)]) {
        auto it =
            std::find(totallCellList.begin(), totallCellList.end(), neighbor);
...
ContactSearch::findContactPairs(
  ...
  for (auto particleIter = particle_handler.begin();
       particleIter != particle_handler.end(); ++particleIter) {

    currrentCell = GridTools::find_active_cell_around_point(
        tr, particleIter->get_location());
    auto it1 =
    ...
    for (auto cellIt = cellNeighborList[index].begin();
         cellIt != cellNeighborList[index].end(); cellIt++) {
      ...
      for (typename Particles::ParticleHandler<
               3, 3>::particle_iterator_range::iterator partIter =
...
void ContactSearch::fineSearch(
  ...
  std::tuple<std::pair<Particles::ParticleIterator<3, 3>,
                       Particles::ParticleIterator<3, 3>>,
             std::vector<double>, double, std::vector<double>, double,
  ...
  for (unsigned int i = 0; i < contactInfo.size(); i++) {
    ...
    if (distance > 0) {
      ...
      if (vecValue(relTangVel) != 0) {
        std::vector<double> tangVec = {relTangVel[0] / vecValue(relTangVel),
                                       relTangVel[1] / vecValue(relTangVel),
                                       relTangVel[2] / vecValue(relTangVel)};
      }

      double tangRelVel = dotProduct(relVel, tangVec);
      double tangOverlap = std::get<8>(contactInfo[i]) +
                           ((dotProduct(std::get<1>(contactInfo[i]),
                                        std::get<6>(contactInfo[i]))) *
                            dt);

      infoTuple = std::make_tuple(
          std::make_pair(contactPairs[i].first, contactPairs[i].second), relVel,
          distance, normVec, normRelVel, relTangVel, tangVec, tangRelVel,
          tangOverlap);
      contactInfo[i] = infoTuple;

      // std::cout<< "the first particle is: " <<
      // (contactInfo[i]).first->get_properties()[0] << " and second particle: "
      // << (contactInfo[i]).second->get_properties()[0]<<std::endl;
      std ::cout << " contact information2: "
                 << "rel vel: " << relVel[0] << " " << relVel[1] << " "
                 << relVel[2] << std::endl;
      std ::cout << " contact information2: "
                 << "distance: " << distance << std::endl;
      std ::cout << " contact information2: "
                 << "normVec: " << normVec[0] << " " << normVec[1] << " "
                 << normVec[2] << std::endl;
      std ::cout << " contact information2: "
                 << "normRelVel: " << normRelVel << std::endl;

      std ::cout << " contact information2: "
                 << "relTangVel: " << relTangVel[0] << " " << relTangVel[1]
                 << " " << relTangVel[2] << std::endl;
      std ::cout << " contact information2: "
                 << "tangRelVel: " << tangRelVel << std::endl;
      std ::cout << " contact information2: "
                 << "tangVec: " << tangVec[0] << " " << tangVec[1] << " "
                 << tangVec[2] << std::endl;
      std ::cout << " contact information2: "
                 << "tangOverlap: " << tangOverlap << std::endl;
  ...
  for (unsigned int i = 0; i < contactPairs.size(); i++) {
    ...
    if (it4 == searchPair.end()) {
      if (distance > 0) {
        ...
        infoTuple = std::make_tuple(
            std::make_pair(contactPairs[i].first, contactPairs[i].second),
            relVel, distance, normVec, normRelVel, relTangVel, tangVec,
            tangRelVel, tangOverlap);
        contactInfo.push_back(infoTuple);

        std::cout << "the first particle is: "
                  << contactPairs[i].first->get_properties()[0]
                  << " and second particle: "
                  << contactPairs[i].second->get_properties()[0] << std::endl;
...
std::vector<double> ContactSearch::normVector(Point<3> point1,
                                              Point<3> point2) {
  Point<3> point3;
...
std::vector<double> ContactSearch::crossProduct(std::vector<double> A,
                                                std::vector<double> B) {
  return {A[1] * B[2] - A[2] * B[1], A[2] * B[0] - A[0] * B[2],
          A[0] * B[1] - A[1] * B[0]};
}

std::vector<double> ContactSearch::vecSubtract(std::vector<double> A,
                                               std::vector<double> B) {
  return {A[0] - B[0], A[1] - B[1], A[2] - B[2]};
}

std::vector<double> ContactSearch::vecAdd(std::vector<double> A,
                                          std::vector<double> B) {
  return {A[0] + B[0], A[1] + B[1], A[2] + B[2]};
}
