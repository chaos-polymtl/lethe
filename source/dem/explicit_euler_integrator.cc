  ...
  const std::vector<double>       &MOI)
{
  for (auto particle = particle_handler.begin();
       ...
       ++particle)
    {
      ...
      double mass_inverse = 1 / particle_properties[PropertiesIndexEnum<solver_type>::mass];
      ...
      for (int d = 0; d < 3; ++d)
        {
          ...
          particle_properties[PropertiesIndexEnum<solver_type>::v_x + d] += dt * acceleration[d];

          // Position integration
          particle_position[d] +=
            dt * particle_properties[PropertiesIndexEnum<solver_type>::v_x + d];
