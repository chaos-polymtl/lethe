...
template <class T> bool existIn(T i, T j) { return (i == j); }

template <class T> bool greatherthan(T i, T j) { return (i > j); }

int main(int argc, char *argv[]) {

  // considering id, type, diameter, density, x, y, z, vx, vy, vz, ax, ay, az,
  // fx, fy, fz, wx, wy, wz, each particle has 19 properties and 9 fields.
  ...
  Particles::ParticleHandler<3, 3> particle_handler(tr, mappinggg,
                                                    n_properties);
  ...
  while (DEM_step < readInput.tInsertion) {
    ...
    if (nPart < readInput.nTotal) // number < total number
    {
      if (fmod(DEM_step, readInput.insertFrequncy) == 1) {
        ins1.uniformInsertion(particle_handler, tr, readInput, nPart, pool,
                              particle);
      }
    }
    ...
    if (fmod(DEM_step, readInput.writeFrequency) == 1) {
      ...
      visObj.build_patches(particle_handler, n_fileds, n_properties,
                           properties);
    ...
    iter1.engine(nPart, particle_handler, tr, DEM_step, DEM_time, readInput,
                 cellNeighbor, contactInfo);
  ...
  // checke nahayi baraye dorost kar kardane hame chi, aval integration ke az
  // inja call kardanesh hazf shod, badesh contact det
  ...
  /*
     ...
     GeometryInfo<3>::vertices_per_cell; ++v)
                    ...
                    auto v_to_c_d =
     GridTools::vertex_to_cell_centers_directions(tr, v_to_c); auto c_and_p =
     GridTools::find_active_cell_around_point( mappinggg, tr, vv, v_to_c,
     v_to_c_d); auto pcell = c_and_p.first;

                  const Particles::ParticleHandler<3,3>::particle_iterator_range
     particle_range = particle_handler.particles_in_cell(pcell);

                  for (typename
     Particles::ParticleHandler<3,3>::particle_iterator_range::iterator
     ppparticle =particle_range.begin(); ppparticle !=
     particle_range.end();++ppparticle)
                  {
                          ppparticle->get_id();
                          std::cout<<"In cell "<< pcell->id() << " the id of
     particles are: " << ppparticle->get_id()<<std::endl;
  ...
  int n_points;
           ...
           for (std::size_t i = 0; i < n_points; ++i)
            ...
            auto v_to_c   = GridTools::vertex_to_cell_map(tr);
            auto v_to_c_d = GridTools::vertex_to_cell_centers_directions(tr,
  v_to_c);
