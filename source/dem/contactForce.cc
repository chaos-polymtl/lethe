  ...
  for (unsigned int i = 0; i < contactInfo.size(); i++) {
    {
      ...
      // does it have access to particle handler or I need to loop over particle
      // handler and search for contactInfo particles
...
std::vector<double> ContactForce::crossProduct(std::vector<double> A,
                                               std::vector<double> B) {
  return {A[1] * B[2] - A[2] * B[1], A[2] * B[0] - A[0] * B[2],
          A[0] * B[1] - A[1] * B[0]};
}

std::vector<double> ContactForce::vecSubtract(std::vector<double> A,
                                              std::vector<double> B) {
  return {A[0] - B[0], A[1] - B[1], A[2] - B[2]};
}

std::vector<double> ContactForce::vecAdd(std::vector<double> A,
                                         std::vector<double> B) {
  return {A[0] + B[0], A[1] + B[1], A[2] + B[2]};
}
