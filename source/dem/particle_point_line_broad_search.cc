...
ParticlePointLineBroadSearch<dim>::find_Particle_Point_Contact_Pairs(
  ...
  for (auto cells_with_boundary_points_information =
    ...
    Point<dim> vertex_location = cells_with_boundary_points_information->second;
    ...
    for (typename Particles::ParticleHandler<dim>::particle_iterator_range::
      ...
      // Making the pair and adding it to the particle_point_contact_candidates
      // map. This map is the output of this function
...
ParticlePointLineBroadSearch<dim>::find_Particle_Line_Contact_Pairs(
  ...
  for (auto cells_with_boundary_lines_information =
    ...
    for (typename Particles::ParticleHandler<dim>::particle_iterator_range::
      ...
      // Making the tuple (particle, beginning and ending points of the boundary
      // line) and adding it to the particle_line_contact_candidates map. This
      // map is the output of this function
      particle_line_contact_candidates.insert(
          {contact_candidate_counter,
           std::make_tuple(particles_in_cell_iterator, first_vertex_location,
