...
template <int dim> void DEMSolver<dim>::print_initial_info() {
  pcout
      << "***************************************************************** \n";
...
template <int dim> void DEMSolver<dim>::solve() {
  ...
  while (simulation_control->integrate()) {
    ...
    if (particles_were_inserted || fmod(simulation_control->get_step_number(),
                                        contact_detection_frequency) == 0) {
      localize_contacts<dim>(
          &local_adjacent_particles, &ghost_adjacent_particles,
          &pw_pairs_in_contact, local_contact_pair_candidates,
          ghost_contact_pair_candidates, pw_contact_candidates);
    }

    if (particles_were_inserted || fmod(simulation_control->get_step_number(),
                                        contact_detection_frequency) == 0) {
      locate_local_particles_in_cells<dim>(
          particle_handler, particle_container, ghost_adjacent_particles,
          local_adjacent_particles, pw_pairs_in_contact,
          particle_points_in_contact, particle_lines_in_contact);
    } else {
      locate_ghost_particles_in_cells<dim>(
          particle_handler, ghost_particle_container, ghost_adjacent_particles);
    }
    ...
    if (particles_were_inserted || fmod(simulation_control->get_step_number(),
      ...
      pp_fine_search_object.pp_Fine_Search(
          ...
          local_adjacent_particles, ghost_adjacent_particles,
          particle_container, neighborhood_threshold);
    }

    // Particle-particle contact force
    pp_contact_force_object->calculate_pp_contact_force(
        &local_adjacent_particles, &ghost_adjacent_particles, parameters,
