...
template <int dim> void DEMSolver<dim>::solve() {
  ...
  while (DEM_step < number_of_steps) {
    ...
    if (DEM_step % parameters.simulationControl.write_frequency == 0) {
      ...
      visualization_object.build_patches(particle_handler, properties);
      WriteVTU<dim> writObj;
      writObj.write_master_files(visualization_object, parameters);
      writObj.writeVTUFiles(visualization_object, DEM_step, DEM_time,
                            parameters);
      computing_timer.leave_subsection();
    }
