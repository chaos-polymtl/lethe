...
template <int dim> void DEMSolver<dim>::solve() {
  ...
  while (DEM_step < number_of_steps) {
    iterator_object.engine(
        particle_handler, triangulation, DEM_step, DEM_time, cell_neighbor_list,
        pairs_in_contact_info, boundary_cells_information, pw_pairs_in_contact,
        parameters, g, properties, property_pool, &pp_force_object,
        &pw_force_object, &integrator_object, &pp_broad_search_object,
        &pp_fine_search_object, &pw_broad_search_object,
