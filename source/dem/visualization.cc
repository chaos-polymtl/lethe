...
{
  // We reserve 1 more name than the number of properties, since we will also be writing the ID of the particles
 ...
 dataset_names.emplace_back("ID");
  ...
  for (int field_position = 0 ; field_position < PropertiesIndex::n_properties ; ++field_position)
    {
      ...
      if (particle->has_properties())
        {
          ...
          // We need to offset the data we write by one since we have one more property due to the fact
          // that we save the ID of the particles.
          for (unsigned int property_index ; property_index < PropertiesIndex::n_properties ; ++property_index)
