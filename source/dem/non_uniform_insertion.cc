...
void NonUniformInsertion<dim>::insert(
  ...
  if (remained_particles != 0) {
    ...
    particle_properties =
        this->assign_particle_propertis(dem_parameters, inserted_this_step,
    ...
    particle_handler.insert_global_particles(
        insertion_points, global_bounding_boxes, particle_properties);
...
template <int dim>
std::vector<double> NonUniformInsertion<dim>::create_random_number_container(
    const double &random_number_range, const int &random_number_seed) {
...
template <int dim>
std::vector<Point<dim>> NonUniformInsertion<dim>::assign_insertion_points(
    const DEMSolverParameters<dim> &dem_parameters) {
        ...
        if (particle_counter < inserted_this_step) {
          ...
          position[0] = insertion_information.x_min +
                        ...
                        random_number_vector[particle_counter] *
                            physical_properties.diameter;
          position[1] =
              insertion_information.y_min + (physical_properties.diameter / 2) +
              (j * insertion_information.distance_threshold *
               physical_properties.diameter) +
              random_number_vector[insertion_information.inserted_this_step -
                                   particle_counter] *
                  physical_properties.diameter;
          if (dim == 3) {
