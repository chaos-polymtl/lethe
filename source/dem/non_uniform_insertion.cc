    ...
    for (int j = 0; j < ny; ++j) {
      ...
      for (int k = 0; k < dim_nz; ++k) {
        ...
        if (inserted_sofar_step < insertion_information.inserted_this_step) {
          ...
          // randomness, the random vector is read once from the beginning and
          // once from the end to be used in positions [0] and [1]
...
std::vector<double> NonUniformInsertion<dim>::create_random_number_container(
    const int &inserted_this_step, const double &random_number_range,
