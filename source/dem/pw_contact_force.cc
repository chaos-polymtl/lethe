...
template <int dim>
Tensor<1, dim>
PWContactForce<dim>::calculation_total_torque(Tensor<1, dim> total_force,Point<dim> center_mass,Point<dim> point_on_boundary)
{
    if (dim==3){
...
{
    std::map<unsigned int, Tensor<1, dim>> total_force; std::map<unsigned int, Tensor<1, dim>> total_torque;
            ...
            pw_pairs_in_contact | boost::adaptors::map_values)
    {
                ...
                pairs_in_contact_content | boost::adaptors::map_values)
        {
            ...
            if (normal_overlap > 0)
            {
                    ...
                    Parameters::Lagrangian::ModelParameters::PWContactForceModel::pw_linear)
                {
                    ...
                    Tensor<1, dim> normal_force =
                            (normal_spring_constant * contact_information.normal_overlap -
                             normal_damping_constant * contact_information.normal_relative_velocity) *
                    ...
                    forces_and_torques= std::make_tuple(normal_force,
                                           tangential_force);
                ...
                total_force[contact_information.boundary_id] = total_force[contact_information.boundary_id] + normal_force + tangential_force;
                total_torque[contact_information.boundary_id]=total_torque[contact_information.boundary_id]+
                                                            calculation_total_torque(total_force[contact_information.boundary_id],
    ...
    std::cout <<"\n"<< -total_force[2][0]<<" "<<-total_force[2][1]<<" "<<-total_force[2][2];
    std::cout <<"\n"<< -total_force[3][0]<<" "<<-total_force[3][1]<<" "<<-total_force[3][2];
    std::cout <<"\n"<< -total_force[4][0]<<" "<<-total_force[4][1]<<" "<<-total_force[4][2];
    std::cout <<"\n"<< -total_torque[2][0]<<" "<<-total_torque[2][1]<<" "<<-total_torque[2][2];
    std::cout <<"\n"<< -total_torque[3][0]<<" "<<-total_torque[3][1]<<" "<<-total_torque[3][2];
    std::cout <<"\n"<< -total_torque[4][0]<<" "<<-total_torque[4][1]<<" "<<-total_torque[4][2];
}
