  ...
  for (typename Triangulation<dim>::active_cell_iterator cell =
    ...
    for (unsigned int vertex = 0; vertex < GeometryInfo<dim>::vertices_per_cell;
      ...
      for (const auto &neighbor : v_to_c[cell->vertex_index(vertex)]) {
        ...
        // If the cell (neighbor) is not present in the total_cell_list vector,
        // it will be added as the neighbor of the main cell ("cell") and also
        // to the total_cell_list to avoid repetition for next cells.
...
template class FindCellNeighbors<3, 3>;
