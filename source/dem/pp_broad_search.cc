...
void PPBroadSearch<dim>::find_PP_Contact_Pairs(
  ...
  // First, a loop is defined over the particle handler. In this loop an
  // iterator to each particle is added to the (particle_id)th element (which is
  // a vector) of contact_pair_candidates. Using this method, the first element
  // of contact_pair_candidates vectors is an iterator to the particle itself
  for (auto particle = particle_handler.begin();
       particle != particle_handler.end(); ++particle) {
    unsigned int particle_id = particle->get_id();
    contact_pair_candidates[particle_id].push_back(particle);
  }
    ...
    if (particle_handler.n_particles_in_cell(*cell_neighbor_iterator) > 0) {
      ...
      for (typename Particles::ParticleHandler<dim>::particle_iterator_range::
        ...
        for (; particles_in_main_cell_iterator_two !=
          ...
          contact_pair_candidates[particles_in_main_cell_iterator_one->get_id()]
              .push_back(particles_in_main_cell_iterator_two);
        }
      }
      ...
      for (; cell_neighbor_iterator != cell_neighbor_list_iterator->end();
        ...
        for (typename Particles::ParticleHandler<dim>::particle_iterator_range::
          ...
          for (typename Particles::ParticleHandler<
            ...
            contact_pair_candidates[particles_in_main_cell_iterator->get_id()]
                .push_back(particles_in_neighbor_cell_iterator);
          }
        }
      }
    }
