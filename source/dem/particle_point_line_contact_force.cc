...
template <int dim, DEM::SolverType solver_type>
void
ParticlePointLineForce<dim,solver_type>::calculate_particle_point_contact_force(
  ...
  for (auto pairs_in_contact_iterator =
       ...
       ++pairs_in_contact_iterator)
    {
      ...
      double         normal_overlap =
        ((particle_properties[DEM::PropertiesIndexEnum<solver_type>::dp]) / 2) -
      ...
      if (normal_overlap > 0)
        {
          ...
          particle_velocity[0] = particle_properties[DEM::PropertiesIndexEnum<solver_type>::v_x];
          particle_velocity[1] = particle_properties[DEM::PropertiesIndexEnum<solver_type>::v_y];
          particle_velocity[2] = particle_properties[DEM::PropertiesIndexEnum<solver_type>::v_z];

          Tensor<1, 3> particle_omega;
          particle_omega[0] =
            particle_properties[DEM::PropertiesIndexEnum<solver_type>::omega_x];
          particle_omega[1] =
            particle_properties[DEM::PropertiesIndexEnum<solver_type>::omega_y];
          particle_omega[2] =
            particle_properties[DEM::PropertiesIndexEnum<solver_type>::omega_z];

          // Defining relative contact velocity
          Tensor<1, 3> contact_relative_velocity =
            particle_velocity +
            cross_product_3d((((particle_properties[DEM::PropertiesIndexEnum<solver_type>::dp]) /
                               2) *
                              particle_omega),
                             normal_vector);
          ...
          double model_parameter_sn =
            ...
            sqrt(particle_properties[DEM::PropertiesIndexEnum<solver_type>::dp] *
                 normal_overlap);
          ...
          double normal_spring_constant =
            ...
            sqrt(particle_properties[DEM::PropertiesIndexEnum<solver_type>::dp] / 2 *
                 normal_overlap);
          double normal_damping_constant =
            -1.8257 * model_parameter_beta *
            sqrt(model_parameter_sn *
                 particle_properties[DEM::PropertiesIndexEnum<solver_type>::mass]);
...
Point<3>
ParticlePointLineForce<dim, solverType>::find_projection_point(const Point<3> &point_p,
