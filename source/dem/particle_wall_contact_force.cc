  ...
  for (unsigned int i = 0; i < pwContactInfo.size(); i++)
    {
      ...
      double     yEff =
        pow((((1.0 - pow(vp, 2.0)) / Yp) + ((1.0 - pow(vw, 2.0)) / Yw)), -1.0);
      double kn =
        1.2024 *
          ...
          ethat =
            (-2.0 * log(ew) *
             sqrt(2.0 / 7.0 *
                  std::get<0>(pwContactInfo[i]).first->get_properties()[19] *
                  kt)) /
            (sqrt(pow(3.1415, 2.0) + pow(log(ew), 2.0)));
        }
      ...
      if (tangForce.norm() < (muw * normalForce.norm()))
        {
          totalForce = normalForce + tangForce;
        }
      else
        {
          Point<dim> coulumbTangForce = (-1.0 * muw * normalForce.norm() *
  ...
  for (unsigned int i = 0; i < pwContactInfo.size(); i++)
    {
      ...
      double     yEff =
        pow((((1.0 - pow(vp, 2.0)) / Yp) + ((1.0 - pow(vw, 2.0)) / Yw)), -1.0);
      double gEff = pow(((2.0 * (2.0 - vp) * (1.0 + vp)) / (Yp)) +
                          ((2.0 * (2.0 - vw) * (1.0 + vw)) / (Yw)),
                        -1.0);
      ...
      if (tangForce.norm() < coulumbLimit)
      ...
      else
        {
          Point<dim> coulumbTangForce =
            (-1.0 * coulumbLimit * sgn(std::get<5>(pwContactInfo[i]))) *
            std::get<6>(pwContactInfo[i]);
