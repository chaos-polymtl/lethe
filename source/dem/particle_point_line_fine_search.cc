...
ParticlePointLineFineSearch<dim>::Particle_Point_Fine_Search(
  ...
  for (auto contact_pair_candidates_iterator =
    ...
    if (distance > 0) {
      ...
      // Creating a sample from the particle_point_line_contact_info_struct and
...
ParticlePointLineFineSearch<dim>::Particle_Line_Fine_Search(
    const std::map<int, std::tuple<Particles::ParticleIterator<dim>, Point<dim>,
                                   Point<dim>>>
  ...
  for (auto contact_pair_candidates_iterator =
    ...
    // Get the particle, particle properties and the locations of beginning and
    // ending vertices of the boundary line once to improve efficiency
    auto particle = std::get<0>(*pair_candidates);
    auto particle_properties = particle->get_properties();
    Point<dim> vertex_one_location = std::get<1>(*pair_candidates);
    Point<dim> vertex_two_location = std::get<2>(*pair_candidates);

    // For finding the particle-line distance, the projection of the particle on
    // the line should be obtained
    Point<dim> projection = find_projection_point(
        particle->get_location(), vertex_one_location, vertex_two_location);
    ...
    if (distance > 0) {
      ...
      particle_line_pairs_in_contact.insert({particle->get_id(), contact_info});
    }
  }
...
template <int dim>
Point<dim> ParticlePointLineFineSearch<dim>::find_projection_point(
    Point<dim> point_p, Point<dim> point_a, Point<dim> point_b) {
