...
template <int dim, DEM::SolverType solver_type>
inline void
LagrangianLoadBalancing<dim,solver_type>::check_load_balance_with_sparse_contacts()
{
  if (simulation_control->get_step_number() % dynamic_check_frequency == 0)
    {
...
template <int dim, DEM::SolverType solver_type>
unsigned int
LagrangianLoadBalancing<dim,solver_type>::calculate_total_cell_weight_with_mobility_status(
  const typename parallel::distributed::Triangulation<dim>::cell_iterator &cell,
  const CellStatus status) const
#endif
{
  ...
  double alpha = 1.0;
  if (cell_mobility_status == AdaptiveSparseContacts<dim,solver_type>::static_active ||
      cell_mobility_status == AdaptiveSparseContacts<dim,solver_type>::advected_active)
    {
      alpha = active_status_factor;
    }
  else if (cell_mobility_status == AdaptiveSparseContacts<dim,solver_type>::inactive ||
           cell_mobility_status == AdaptiveSparseContacts<dim,solver_type>::advected)
    {
      alpha = inactive_status_factor;
    }
