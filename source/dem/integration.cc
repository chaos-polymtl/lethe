  ...
  ParametersDEM<3>                  DEMparam)
{
    for (auto particle = particle_handler.begin();
         ...
         ++particle)
      {
        ...
        particle->get_properties()[7] =
          particle->get_properties()[7] + (DEMparam.simulationControl.dt / 2.0) * (axStar + particle->get_properties()[10]);

        particle->get_properties()[8] =
          particle->get_properties()[8] + (DEMparam.simulationControl.dt / 2.0) * (ayStar + particle->get_properties()[11]);
        particle->get_properties()[9] =
          particle->get_properties()[9] + (DEMparam.simulationControl.dt / 2.0) * (azStar + particle->get_properties()[12]);

        // Position integration:
        particle->get_properties()[4] =
          particle->get_properties()[4] + (DEMparam.simulationControl.dt / 2.0) * (vxStar + particle->get_properties()[7]);
        particle->get_properties()[5] =
          particle->get_properties()[5] + (DEMparam.simulationControl.dt / 2.0) * (vyStar + particle->get_properties()[8]);
        particle->get_properties()[6] =
          particle->get_properties()[6] + (DEMparam.simulationControl.dt / 2.0) * (vzStar + particle->get_properties()[9]);
