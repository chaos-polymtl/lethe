  ...
  if (contact_candidates.size() != 0)
    for (auto pairs_in_contact_iterator = pairs_in_contact.begin();
         pairs_in_contact_iterator != pairs_in_contact.end();)
      {
                ...
                if (contact_candidate_element != contact_candidates.end())
                  {
                    ...
                    // new broad search candidates or from the list of the
                    // history of the fine search candidate list depending on
                    // the last checks
                    if (search_iterator !=
                        contact_candidate_element->second.end())
                      {
                        // The current 2nd is a candidate to the current
                        // particle. The 2nd particle is then removed from the
                        // new broad search particle-particle contact candidates
                        // list but kept as history in the fine search list
                        contact_candidate_element->second.erase(
                          search_iterator);

                        ++adjacent_map_iterator;

                        // To prevent searching through the contact_candidates a
                        // second time if there is a deletion in the first
                        // search, we continue here instead of using an if, else
                        ...
                        object_contact_candidates->second.end())
                      {
                        if constexpr (contact_type ==
                                          ...
                                          local_periodic_particle_particle)
                          {
                            ...
                            // local The 2nd particle is then removed from the
                            // new broad search particle-particle contact
                            // candidates list but kept as history in the fine
                            // search list
                        ...
                        if constexpr (contact_type ==
                                      ContactType::ghost_particle_particle)
                          {
                            // The current particle from history list is still a
                            // candidate to the 2nd particle and the contact is
                            // not local The 2nd particle is then removed from
                            // the history in the fine search list
                            // particle-particle contact candidates list but
                            // kept in new broad search list
                            adjacent_map_iterator =
                ...
                if (contact_candidate_element != contact_candidates.end())
                  {
                    ...
                    // Particle-wall/face pair is removed from the list of the
                    // new broad search candidates or from the list of the
                    // history of the fine search candidate list depending on
                    // the last check
                    if (search_iterator !=
                        contact_candidate_element->second.end())
                      {
                        ...
                        // particle-wall/face contact candidates list but kept
                        // as history in the fine search list
                        contact_candidate_element->second.erase(
                          search_iterator);
                        ++adjacent_map_iterator;
                        continue;
        ...
        // Otherwise it is deleted. This is necessary to prevent memory
        // inflation. If this is not done, pairs_in_contact keeps on growing
        // until it reaches the total number of particles in the simulation.
        else
          pairs_in_contact_iterator =
