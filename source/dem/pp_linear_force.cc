...
void PPLinearForce<dim>::calculate_pp_contact_force(
    ...
    const DEMSolverParameters<dim> &dem_parameters, const double &dt) {
  // Looping over adjacent_particles with iterator
    ...
    for (auto adjacent_particles_list_iterator =
      ...
      if (normal_overlap > 0) {
        ...
        this->update_contact_information(
            *contact_info, particle_one_properties, particle_two_properties,
            particle_one_location, particle_two_location, dt);
        ...
        std::tuple<Tensor<1, dim>, Tensor<1, dim>, Tensor<1, dim>,
                   Tensor<1, dim>>
            forces_and_torques =
                this->calculate_linear_contact_force_and_torque(
                    physical_properties, *contact_info, particle_one_properties,
  ...
  // Doing the same calculations for ghost particles:
  // Looping over adjacent_particles with iterator
  // adjacent_particles_iterator
...
PPLinearForce<dim>::calculate_linear_contact_force_and_torque(
  ...
  double normal_damping_constant = sqrt(
      ...
      (1 + pow((3.1415 /
                (log(physical_properties.restitution_coefficient_particle) +
                 DBL_MIN)),
               2)));
  ...
  if (dim == 3) {
    tangential_torque = cross_product_3d(
        (0.5 * particle_one_properties[DEM::PropertiesIndex::dp] *
         contact_info.normal_unit_vector),
        tangential_force);
  }
  ...
  return std::make_tuple(normal_force, tangential_force, tangential_torque,
