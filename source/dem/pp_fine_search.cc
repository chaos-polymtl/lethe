...
PPFineSearch<dim>::PPFineSearch()
    : dummy_timer(std::cout, TimerOutput::summary, TimerOutput::wall_times) {}
...
void PPFineSearch<dim>::pp_Fine_Search(
    ...
    for (auto pairs_in_contact_info_iterator = pairs_in_contact_info.begin();
      ...
      for (auto map_iterator = contact_partners_map->begin();
        ...
        if (overlap_distance > 0) {
          ...
          // Adding a small value to the tangential_overlap_norm to avoid 0/0
          // occurance
  ...
  {
    // Now iterating over contact_pair_candidates (map of vectors), which is the
    // output of broad search. Note that the first element of each vector of
    // contact_pair_candidates is an iterator to the first particle itself. If a
    // pair is in contact (distance > 0) and does not exist in the
    // pairs_in_contact, it is added to the pairs_in_contact

    for (auto candidate_map_iterator = contact_pair_candidates.begin();
         candidate_map_iterator != contact_pair_candidates.end();
         ++candidate_map_iterator) {
      // Getting the particle contact list
      auto contact_candidates_vector = candidate_map_iterator->second;

      // As mentioned, based on definition, particle one is the first element of
      // each vector
      auto particle_one = *contact_candidates_vector.begin();

      // Now iterating over the rest of vector elements, which are the contact
      // candidates of particle one

      for (auto particle_candidate = contact_candidates_vector.begin() + 1;
           particle_candidate != contact_candidates_vector.end();
           ++particle_candidate) {

        // Get particles one and two from the vector and the total array view to
        // the particle properties once to improve efficiency
        auto particle_two = *particle_candidate;
        auto particle_one_properties = particle_one->get_properties();
        auto particle_two_properties = particle_two->get_properties();
