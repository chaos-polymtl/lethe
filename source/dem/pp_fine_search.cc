...
void PPFineSearch<dim>::pp_Fine_Search(
  ...
  for (auto pairs_in_contact_info_iterator = pairs_in_contact_info.begin();
    ...
    // Iterating over each map which contains the contact information including
    // particles I and II

    for (auto map_iterator = pairs_in_contact_info_iterator->begin();
         map_iterator != pairs_in_contact_info_iterator->end();) {

      // Getting contact information and particles I and II as local variables
      ...
      if (distance > 0.0) {
        ...
        /*
           ...
           / 2.0) * particle_one_omega) +
                           ((particle_two_properties[DEM::PropertiesIndex::dp]
           / 2.0) * particle_two_omega)), normal_vector));
                }
                */

        // Calculation of normal relative velocity. Note that in the following
        // line the product acts as inner product since both sides are
        // vectors, while in the second line the product is scalar and vector
        // product
  ...
  for (auto contact_pair_candidates_iterator = contact_pair_candidates.begin();
    ...
    if (distance > 0.0) {
      ...
      // vector as well as
      // (particle_two_properties[DEM::PropertiesIndex::id])th element of the
      // vector. If the pair does not exist in these two locations, it should
      // be added to pairs_in_contact
