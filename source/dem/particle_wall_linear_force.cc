...
template <int dim, DEM::SolverType solver_type>
void
ParticleWallLinearForce<dim,solver_type>::calculate_particle_wall_contact_force(
...
template <int dim, DEM::SolverType solver_type>
void
ParticleWallLinearForce<dim,solver_type>::calculate_particle_floating_wall_contact_force(
...
template <int dim, DEM::SolverType solver_type>
std::tuple<Tensor<1, 3>, Tensor<1, 3>, Tensor<1, 3>, Tensor<1, 3>>
ParticleWallLinearForce<dim,solver_type>::calculate_linear_contact_force_and_torque(
  ...
  const ArrayView<const double>   &particle_properties)
{
  ...
  double rp_sqrt = sqrt(particle_properties[DEM::PropertiesIndexEnum<solver_type>::dp] * 0.5);

  double normal_spring_constant =
    1.0667 * rp_sqrt * this->effective_youngs_modulus[particle_type] *
    pow((0.9375 * particle_properties[DEM::PropertiesIndexEnum<solver_type>::mass] * 1.0 *
  ...
  double normal_damping_constant =
    ...
    sqrt(particle_properties[DEM::PropertiesIndexEnum<solver_type>::mass] *
