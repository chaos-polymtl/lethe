...
FindBoundaryCellsInformation<dim, spacedim>::find_boundary_cells_information(
  ...
  // Initilizing a quadrate on the system. This will be used to obtain a sample
  // point on the boudnary faces
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell_iterator =
           tr.begin_active();
       cell_iterator != tr.end(); ++cell_iterator) {
    ...
    for (unsigned int face_id = 0; face_id < GeometryInfo<dim>::faces_per_cell;
      ...
      if (cell_iterator->face(face_id)->at_boundary() == true) {
        ...
        for (unsigned int f_q_point = 0; f_q_point < n_face_q_points;
          ...
          Point<dim> quad_point = fe_face_values.quadrature_point(dim);

          // Boundary id
          int boundary_id = cell_iterator->face(face_id)->boundary_id();

          // Storing these information into the boundary_cells_info_struct
          ...
          // Searching to see if boundary with these information were already
          // found or not. If this is a new boundary face, it will be added to
          // the output_vector as well as the search_vector to avoid repetition
          auto information_search_element =
              std::make_pair(boundary_id, cell_iterator);
          auto search_iterator =
              std::find(search_vector.begin(), search_vector.end(),
                        information_search_element);
          if (search_iterator == search_vector.end()) {
            output_vector.push_back(boundary_information);
            search_vector.push_back(std::make_pair(boundary_id, cell_iterator));
          }
        }
      }
    }
