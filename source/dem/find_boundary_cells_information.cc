...
FindBoundaryCellsInformation<dim>::find_boundary_cells_information(
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell_iterator =
       ...
       cell_iterator != triangulation.end(); ++cell_iterator) {
      ...
      if (cell_iterator->face(face_id)->at_boundary()) {
        ...
        for (unsigned int f_q_point = 0; f_q_point < n_face_q_points;
          ...
          auto information_search_element =
              std::make_pair(face_id, cell_iterator);
          ...
          if (search_iterator == search_vector.end()) {
            ...
            boundary_cells_with_faces.push_back(cell_iterator);
...
template <int dim>
void FindBoundaryCellsInformation<dim>::
  ...
  for (const auto &face : triangulation.active_face_iterators()) {
    if (face->at_boundary()) {
      for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_face; ++v) {
        boundary_vertices.insert({face->vertex_index(v), face->vertex(v)});
      }
    }
  }
  ...
  for (auto iterator = boundary_vertices.begin();
       iterator != boundary_vertices.end(); ++iterator) {
    ...
    for (unsigned int counter = 0; counter != candidate_cells.size();
         ++counter) {
      auto search_iterator =
          std::find(boundary_cells_with_faces.begin(),
                    boundary_cells_with_faces.end(), candidate_cells[counter]);
      if (search_iterator == boundary_cells_with_faces.end()) {
        boundary_cells_with_lines_or_points.push_back(candidate_cells[counter]);
      }
    }
  }
  ...
  for (unsigned int counter = 0;
       counter != boundary_cells_with_lines_or_points.size(); ++counter) {
