...
ParticleInsertion::ParticleInsertion(ReadInputScript readInput) {
  ...
  n_exp = int((readInput.ins_x_max - readInput.ins_x_min) /
              (2 * readInput.diameter)) *
          int((readInput.ins_y_max - readInput.ins_y_min) /
              (2 * readInput.diameter)) *
          int((readInput.ins_z_max - readInput.ins_z_min) /
              (2 * readInput.diameter));
...
void ParticleInsertion::uniformInsertion(
    ...
    const Triangulation<3, 3> &tr, ReadInputScript readInput, int &nPart,
    Particles::PropertyPool &pool, Particles::Particle<3> &particle) {
  /// typename Particles::PropertyPool::Handle handle =
  /// pool.allocate_properties_array();
  int nx = int((readInput.ins_x_max - readInput.ins_x_min) /
               (2 * readInput.diameter));
  int ny = int((readInput.ins_y_max - readInput.ins_y_min) /
               (2 * readInput.diameter));
  int nz = int((readInput.ins_z_max - readInput.ins_z_min) /
               (2 * readInput.diameter));
  int nP = 0;
      ...
      for (int k = 0; k < nz; ++k)
        if (nP < readInput.nInsert) {
          ...
          if (id == 99) {
            ...
            std::cout << GridTools::find_active_cell_around_point(
