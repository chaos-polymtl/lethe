...
void DEM_iterator::engine(int &nPart, Particles::ParticleHandler<3,3> &particle_handler, const Triangulation<3,3> &tr, int &step, float &time, ReadInputScript readInput, std::pair<std::vector<std::set<Triangulation<3>::active_cell_iterator>>,std::vector<Triangulation<3>::active_cell_iterator>> cellNeighbor, std::vector<std::tuple<std::pair<Particles::ParticleIterator<3,3>, Particles::ParticleIterator<3, 3>>, std::vector<double>, double, std::vector<double>, double, std::vector<double>, std::vector<double>, double, double>> &contactInfo, std::vector<std::tuple<int, Triangulation<3>::active_cell_iterator, int, Point<3>, Point<3>>> boundaryCellInfo, std::vector<std::tuple<std::pair<Particles::ParticleIterator<3,3>, int>, Point<3>, Point<3>, double, double, double, Point<3>, double >> &pwContactInfo)
{
	//moving walls
...
//
	...
	//			std:: cout <<"position: " << particle->get_properties()[4]<< " " <<  particle->get_properties()[5] << " " << particle->get_properties()[6] << std::endl;
	//			}
