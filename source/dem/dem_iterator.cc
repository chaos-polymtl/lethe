...
void DEM_iterator<dim, spacedim>::engine(
    int &nPart, Particles::ParticleHandler<dim, spacedim> &particle_handler,
    ...
    Particles::PropertyPool &propPool, ContactSearch<dim, spacedim> cs,
    ...
    int tInsertion, int nInsert, int insertFrequency, float x_min, float y_min,
    float z_min, float x_max, float y_max, float z_max, int numFields,
    int numProperties) {
  // moving walls
  ...
  if (fmod(step, insertFrequency) == 1) {
    if (step < tInsertion) {
      ...
      {
        ParticleInsertion<dim, spacedim> ins1(x_min, y_min, z_min, x_max, y_max,
                                              z_max, dp, nInsert);
        ins1.nonUniformInsertion(particle_handler, tr, nPart, propPool, x_min,
                                 y_min, z_min, x_max, y_max, z_max, dp, nInsert,
                                 rhop, g);
      }
    }
  }
  ...
  // contact force
  cf.nonLinearCF(inContactInfo, Yp, vp, mup, murp);
