...
template <int dim>
void PPContactForce<dim>::update_contact_information(
...
// pair
template <int dim>
void PPContactForce<dim>::apply_force_and_torque_real(
...
// pair
template <int dim>
void PPContactForce<dim>::apply_force_and_torque_ghost(
    ArrayView<double> &particle_one_properties,
    const std::tuple<Tensor<1, dim>, Tensor<1, dim>, Tensor<1, dim>,
                     Tensor<1, dim>> &forces_and_torques) {
  // Getting the values from the forces_and_torques tuple, which are: 1, normal
