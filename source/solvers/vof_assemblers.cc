  ...
  StabilizedMethodsCopyData                    &copy_data)
{
  ...
  const double       diffusion_factor =
    this->vof_parameters.surface_tension_force.curvature_diffusion_factor;
  // Copy data elements
  auto &local_matrix = copy_data.local_matrix;
  // Assemble local matrix
  for (unsigned int q = 0; q < n_q_points; ++q)
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      const Tensor<1, dim> projected_vof_phase_gradient =
        scratch_data.present_phase_gradient_projection_values[q];
      ...
      for (unsigned int i = 0; i < n_dofs; ++i)
        {
          ...
          local_rhs(i) +=
            grad_phi_i *
            (projected_vof_phase_gradient / projected_vof_phase_gradient_norm) *
            JxW;
        }
    }
}

template class VOFAssemblerCurvatureProjection<2>;
template class VOFAssemblerCurvatureProjection<3>;
