...
template <int dim>
void
VOFCurvatureProjectionScratchData<dim>::allocate()
{
  // Initialize size of arrays
  this->n_q_points = fe_values_curvature_projection.get_quadrature().size();
  this->n_dofs     = fe_values_curvature_projection.get_fe().n_dofs_per_cell();

  // Initialize solution arrays
  this->present_curvature_projection_values =
    std::vector<double>(this->n_q_points);
  this->present_curvature_projection_gradients =
    std::vector<Tensor<1, dim>>(this->n_q_points);

  // Initialize extractor
  this->phase_fraction_gradients.first_vector_component = 0;

  // Initialize VOF phase gradient projection solution
  this->present_phase_gradient_projection_values =
    std::vector<Tensor<1, dim>>(this->n_q_points);
...
template class VOFCurvatureProjectionScratchData<2>;
template class VOFCurvatureProjectionScratchData<3>;
