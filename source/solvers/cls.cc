...
ConservativeLevelSet<dim>::ConservativeLevelSet(
  ...
  : AuxiliaryPhysics<dim, GlobalVectorType>(
      p_simulation_parameters.physics_solving_strategy.at(PhysicsID::CLS))
        ...
        .ids_and_isocontours_per_variable.count(Variable::phase))
{
  ...
  this->cls_subequations_interface =
    std::make_shared<CLSSubequationsInterface<dim>>(this->simulation_parameters,
                                                    this->pcout,
                                                    this->triangulation,
  ...
  if (simulation_parameters.multiphysics.cls_parameters.reinitialization_method
      ...
      simulation_parameters.initial_condition
        ...
        Parameters::CLSInitialConditionType::geometric)
    {
      /* For the CLS solver, the interface is defined as the iso-contour
...
ConservativeLevelSet<dim>::setup_assemblers()
{
  ...
  if (time_stepping_is_bdf(this->simulation_control->get_assembly_method()))
    {
      this->assemblers.emplace_back(
        std::make_shared<CLSAssemblerBDF<dim>>(this->simulation_control));
    }
  ...
  if (simulation_parameters.fem_parameters.CLS_uses_dg)
    ...
    {
      // Core assembler
      this->assemblers.emplace_back(std::make_shared<CLSAssemblerCore<dim>>(
        this->simulation_control,
      ...
      if (this->simulation_parameters.stabilization.cls_dcdd_stabilization)
        this->assemblers.emplace_back(
          std::make_shared<CLSAssemblerDCDDStabilization<dim>>(
            this->simulation_control,
...
ConservativeLevelSet<dim>::assemble_system_matrix_cg()
{
  ...
  auto scratch_data =
    CLSScratchData<dim>(this->simulation_control,
  ...
  const auto face_worker =
    [&](const typename DoFHandler<dim>::active_cell_iterator &cell,
      ...
      copy_data_face.joint_dof_indices =
        scratch_data.fe_interface_values_cls.get_interface_dof_indices();
...
ConservativeLevelSet<dim>::gather_output_hook()
{
  ...
  auto cls_parameters = this->simulation_parameters.multiphysics.cls_parameters;
      ...
      if (cell->is_locally_owned())
        {
          ...
          fe_values_cls.get_function_values(*this->present_solution,
                                            phase_values);

          // Get the exact solution at all gauss points
          exact_solution.value_list(fe_values_cls.get_quadrature_points(),
          ...
          for (unsigned int q = 0; q < n_q_points; q++)
            {
              ...
              l2error += (sim - exact) * (sim - exact) * fe_values_cls.JxW(q);
            }
        }
    }
  ...
  for (const auto &cell : this->dof_handler->active_cell_iterators())
    {
      if (cell->is_locally_owned())
        {
          fe_values_cls.reinit(cell);
          quadrature_locations = fe_values_cls.get_quadrature_points();
          ...
          for (unsigned int q = 0; q < n_q_points; q++)
            {
              const double JxW = fe_values_cls.JxW(q);
  ...
  const Parameters::FluidIndicator monitored_fluid)
{
  ...
  for (const auto &cell_cls : this->dof_handler->active_cell_iterators())
    {
      if (cell_cls->is_locally_owned())
        {
                  ...
                  case Parameters::FluidIndicator::fluid0:
                    {
                      ...
                      this->mass_monitored += fe_values_cls.JxW(q) *
                                              (1 - phase_values[q]) *
                                              density_0[q];
                      break;
                    }
                  case Parameters::FluidIndicator::fluid1:
                    {
                      ...
                      this->mass_monitored +=
                        fe_values_cls.JxW(q) * phase_values[q] * density_1[q];
                      break;
                    }
                  default:
  ...
  const bool temperature_dependent)
{
  ...
  // Initialize CLS FEValues and get number of quadrature points
  FEValues<dim>      fe_values_cls(*this->mapping,
                              *this->fe,
                              *this->cell_quadrature,
      ...
      if (cell_cls->is_locally_owned())
        {
          ...
          double cell_measure =
            compute_cell_measure_with_JxW(fe_values_cls.get_JxW_values());
      ...
      if (this_mpi_process == 0)
        {
          ...
          if (this->simulation_control->is_steady())
          ...
          else
            {
              ...
              this->table_monitoring_cls.set_precision(
                "time", this->simulation_control->get_log_precision());
            }
        }

      double geometric_volume_outside, geometric_volume_inside, surface;

      // The mesh classifier in the InterfaceTools::integrate_volume_and_surface
          ...
          if (this_mpi_process == 0)
            {
              ...
              this->table_monitoring_cls.add_value(volume_column_name,
                                                   this->volume_monitored);
              ...
              if (!simulation_parameters.fem_parameters.CLS_uses_dg)
                {
                  this->table_monitoring_cls.add_value(area_column_name,
                                                       surface);
              ...
              for (int d = 0; d < dim; ++d)
                {
                  this->table_monitoring_cls.add_value(momentum_names[d],
                                                       momentum[d]);
      ...
      if (this_mpi_process == 0)
        {
          ...
          if (this->simulation_control->get_iteration_number() %
              ...
              0)
            {
              ...
              this->table_monitoring_cls.write_text(output);
            }
      ...
      if (this_mpi_process == 0)
        {
          if (simulation_parameters.post_processing.verbosity ==
              Parameters::Verbosity::verbose)
            {
              ...
              std::vector<std::string> dependent_column_names;
              dependent_column_names.emplace_back("x_cls");
              dependent_column_names.emplace_back("y_cls");
              if constexpr (dim == 3)
                dependent_column_names.emplace_back("z_cls");
              dependent_column_names.emplace_back("vx_cls");
              dependent_column_names.emplace_back("vy_cls");
              if constexpr (dim == 3)
              ...
              std::vector<std::vector<Tensor<1, dim>>>
                position_and_velocity_vectors{position_vector,
                                              velocity_vector,
                                              position_vector_geo,
                                              velocity_vector_geo};
          ...
          this->table_barycenter.set_precision(
            "time", this->simulation_control->get_log_precision());
          ...
          if constexpr (dim == 3)
            {
              ...
              this->table_barycenter.set_precision(
                "z_cls", this->simulation_control->get_log_precision());
            }
          ...
          if constexpr (dim == 3)
            {
              ...
              this->table_barycenter.set_precision(
                "vz_cls", this->simulation_control->get_log_precision());
            }
          ...
          this->table_barycenter.set_precision(
            "x_cls_geo", this->simulation_control->get_log_precision());
          ...
          this->table_barycenter.set_precision(
            "y_cls_geo", this->simulation_control->get_log_precision());

          if constexpr (dim == 3)
            {
              this->table_barycenter.add_value(
                "z_cls_geo", position_and_velocity_geo.first[2]);
              this->table_barycenter.set_scientific("z_cls_geo", true);
              this->table_barycenter.set_precision(
                "z_cls_geo", this->simulation_control->get_log_precision());
            }
          ...
          this->table_barycenter.set_precision(
            "vx_cls_geo", this->simulation_control->get_log_precision());
          ...
          this->table_barycenter.set_precision(
            "vy_cls_geo", this->simulation_control->get_log_precision());

          if constexpr (dim == 3)
            {
              this->table_barycenter.add_value(
                "vz_cls_geo", position_and_velocity_geo.second[2]);
              this->table_barycenter.set_scientific("vz_cls_geo", true);
              this->table_barycenter.set_precision(
                "vz_cls_geo", this->simulation_control->get_log_precision());
            }
  ...
  GlobalVectorType &solution)
{
  ...
  SolverControl solver_control(
    this->simulation_parameters.linear_solver.at(PhysicsID::CLS).max_iterations,
  ...
  const unsigned int ilu_fill =
    this->simulation_parameters.linear_solver.at(PhysicsID::CLS)
      .ilu_precond_fill;
...
ConservativeLevelSet<dim>::write_checkpoint()
{
  ...
  // Serialize all post-processing tables that are currently used with the CLS
  // solver
...
ConservativeLevelSet<dim>::setup_dofs()
{
  ...
  this->cls_subequations_interface->setup_dofs();
...
ConservativeLevelSet<dim>::solve_linear_system()
{
  ...
  const double absolute_residual =
    simulation_parameters.linear_solver.at(PhysicsID::CLS).minimum_residual;
  const double relative_residual =
    simulation_parameters.linear_solver.at(PhysicsID::CLS).relative_residual;
  ...
  if (this->simulation_parameters.linear_solver.at(PhysicsID::CLS).verbosity !=
  ...
  SolverControl solver_control(
    simulation_parameters.linear_solver.at(PhysicsID::CLS).max_iterations,
  ...
  TrilinosWrappers::SolverGMRES::AdditionalData solver_parameters(
    false,
    simulation_parameters.linear_solver.at(PhysicsID::CLS).max_krylov_vectors);
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              ...
              for (unsigned int k = 0; k < dofs_per_cell; ++k)
                {
                  phi_phase[k] = fe_values_cls.shape_value(k, q);
                }
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  // Matrix assembly
  ...
  TrilinosWrappers::SparseMatrix &mass_matrix)
{
  ...
  FEValues<dim> fe_values_cls(*this->fe,
                              quadrature_formula,
                              update_values | update_JxW_values);
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
              cell_matrix(i, i) +=
                (fe_values_cls.shape_value(i, q) *
                 fe_values_cls.shape_value(i, q) * fe_values_cls.JxW(q));
          cell->get_dof_indices(local_dof_indices);
          this->nonzero_constraints.distribute_local_to_global(
  ...
  GlobalVectorType       &filtered_solution)
{
  ...
  if (this->simulation_parameters.multiphysics.cls_parameters.phase_filter
...
ConservativeLevelSet<dim>::reinitialize_interface_with_pde_based_method()
{
  ...
  if (this->simulation_parameters.multiphysics.cls_parameters
        .reinitialization_method.frequency > 1)
    {
      ...
      // Set CLS information in the CLS subequations interface
      this->cls_subequations_interface->set_cls_solution_and_dof_handler(
