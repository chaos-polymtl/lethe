...
template <int dim>
void
VOFPhaseGradientProjection<dim>::compute_normalized_vector_solution()
{
  // Initializations
  auto mpi_communicator = this->triangulation->get_mpi_communicator();
  GlobalVectorType phase_gradient_owned(this->locally_relevant_dofs,
                                        mpi_communicator);
  GlobalVectorType normalized_phase_gradient_owned(this->locally_owned_dofs,
                                                   mpi_communicator);
  phase_gradient_owned = *this->present_solution;

  // Resize
  this->present_normalized_solution->reinit(*this->present_solution);

  // Get number of DoFs per cell
  const unsigned int n_dofs_per_cell = this->fe->dofs_per_cell;
  const unsigned int base_n_dofs_per_cell =
    this->fe->base_element(0).dofs_per_cell;

  // Container of global dof indices of the cell
  ...
  for (const auto &cell : this->dof_handler->active_cell_iterators())
    {
      ...
      std::vector<std::array<types::global_dof_index, dim>> global_indices(
        base_n_dofs_per_cell);
      std::vector<Tensor<1, dim>> phase_gradient_tensors(base_n_dofs_per_cell);

      // Normalize only DoFs of locally owned cells
      if (cell->is_locally_owned())
        {
          // Get global dof indices of the cell
          cell->get_dof_indices(cell_dof_indices);

          // Get phase gradients and associated global dof indices
          for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
            {
              const unsigned int component =
                this->fe->system_to_component_index(i).first;
              const unsigned int base_index =
                this->fe->system_to_component_index(i).second;
              const auto global_index = cell_dof_indices[i];

              global_indices[base_index][component] = global_index;
              phase_gradient_tensors[base_index][component] =
                phase_gradient_owned[global_index];
            }

          // Normalize phase gradients of locally owned DoFs only
          for (unsigned int base_index = 0; base_index < base_n_dofs_per_cell;
               ++base_index)
            {
              const double phase_gradient_norm =
                phase_gradient_tensors[base_index].norm() + tol;
              const double phase_gradient_norm_inv = 1 / phase_gradient_norm;

              for (int d = 0; d < dim; ++d)
                {
                  const auto global_index = global_indices[base_index][d];

                  // Check if DoF is locally owned, and if so, normalize it
                  if (normalized_phase_gradient_owned.locally_owned_elements()
                        .is_element(global_index))
                    {
                      normalized_phase_gradient_owned[global_index] =
                        phase_gradient_tensors[base_index][d] *
                        phase_gradient_norm_inv;
                    }
                }
            }
        }
  ...
  // Insert computed normal vector in corresponding solution vector
  normalized_phase_gradient_owned.compress(VectorOperation::insert);
  *this->present_normalized_solution = normalized_phase_gradient_owned;
  this->present_normalized_solution->update_ghost_values();
}
