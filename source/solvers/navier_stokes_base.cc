...
template <int dim, typename VectorType, typename DofsType>
void
NavierStokesBase<dim, VectorType, DofsType>::write_sliced_results(const VectorType &solution)
{
  TimerOutput::Scope t(this->computing_timer, "Output VTU");
  ...
  parallel::distributed::Triangulation<dim-1> patch_tria;
  const MappingQ<dim-1>                       patch_mapping;
  std::vector<int> repetitions(dim - 1, 4);
  GridGenerator::hyper_rectangle(patch_tria, repetitions, Point<dim-1>(0., 0.), Point<dim-1>(1., 1.));
