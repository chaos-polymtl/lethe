...
{
  flow_rate = calculate_flow_rate(dof_handler,
                                  evaluation_point,
  ...
  if (nsparam.flow_control.verbosity == Parameters::Verbosity::verbose &&
      simulation_control->get_step_number() > 0 && this_mpi_process == 0)
    {
      std::cout << "+------------------------------------------+" << std::endl;
...
NavierStokesBase<dim, VectorType, DofsType>::dynamic_flow_control()
{
  if (nsparam.flow_control.enable_flow_control &&
      nsparam.simulation_control.method !=
        Parameters::SimulationControl::TimeSteppingMethod::steady)
    {
      flow_control.calculate_beta(flow_rate,
  ...
  const VectorType &evaluation_point)
{
  ...
  forces_on_boundaries = calculate_forces(dof_handler,
                                          evaluation_point,
      ...
      this_mpi_process == 0)
    {
      ...
      TableHandler table =
        make_table_scalars_tensors(nsparam.boundary_conditions.id,
                                   ...
                                   nsparam.simulation_control.log_precision);
  ...
  for (unsigned int i_boundary = 0;
       ...
       ++i_boundary)
    {
      forces_tables[i_boundary].add_value(
        "time", simulation_control->get_current_time());
      forces_tables[i_boundary].add_value("f_x",
                                          forces_on_boundaries[i_boundary][0]);
      forces_tables[i_boundary].add_value("f_y",
                                          forces_on_boundaries[i_boundary][1]);
      if (dim == 3)
        forces_tables[i_boundary].add_value(
          "f_z", forces_on_boundaries[i_boundary][2]);
      else
        forces_tables[i_boundary].add_value("f_z", 0.);

      // Precision
      forces_tables[i_boundary].set_precision(
  ...
  const VectorType &evaluation_point)
{
  ...
  torques_on_boundaries = calculate_torques(dof_handler,
                                            evaluation_point,
                                            nsparam.physical_properties,
                                            nsparam.fem_parameters,
                                            nsparam.boundary_conditions,
  ...
  for (unsigned int boundary_id = 0;
       ...
       ++boundary_id)
    {
      torques_tables[boundary_id].add_value(
        "time", simulation_control->get_current_time());
      torques_tables[boundary_id].add_value(
...
NavierStokesBase<dim, VectorType, DofsType>::finish_time_step()
{
  if (nsparam.simulation_control.method !=
      Parameters::SimulationControl::TimeSteppingMethod::steady)
    {
      ...
      const double CFL       = calculate_CFL(dof_handler,
                                       present_solution,
                                       simulation_control->get_time_step(),
                                       mpi_communicator);
      simulation_control->set_CFL(CFL);
    }
  if (nsparam.restart_parameters.checkpoint &&
      simulation_control->get_step_number() %
          nsparam.restart_parameters.frequency ==
        0)
    {
      write_checkpoint();
    }

  if (nsparam.timer.type == Parameters::Timer::Type::iteration)
    {
...
NavierStokesBase<dim, VectorType, DofsType>::refine_mesh()
{
  if (simulation_control->get_step_number() %
        nsparam.mesh_adaptation.frequency ==
      0)
    {
      ...
      else if (nsparam.mesh_adaptation.type ==
...
NavierStokesBase<dim, VectorType, DofsType>::refine_mesh_kelly()
{
  ...
  const MappingQ<dim> mapping(velocity_fem_degree,
                              nsparam.fem_parameters.qmapping_all);
  const FEValuesExtractors::Vector velocity(0);
  const FEValuesExtractors::Scalar pressure(dim);
  auto &present_solution = this->get_present_solution();
  if (nsparam.mesh_adaptation.variable ==
      Parameters::MeshAdaptation::Variable::pressure)
    {
      KellyErrorEstimator<dim>::estimate(
        mapping,
        dof_handler,
        QGauss<dim - 1>(number_quadrature_points + 1),
        typename std::map<types::boundary_id, const Function<dim, double> *>(),
        present_solution,
        estimated_error_per_cell,
        fe.component_mask(pressure));
    }
  else if (nsparam.mesh_adaptation.variable ==
  ...
  if (nsparam.mesh_adaptation.fractionType ==
      Parameters::MeshAdaptation::FractionType::number)
    parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(
      tria,
      estimated_error_per_cell,
      nsparam.mesh_adaptation.refinement_fraction,
      nsparam.mesh_adaptation.coarsening_fraction,
      nsparam.mesh_adaptation.maximum_number_elements);

  else if (nsparam.mesh_adaptation.fractionType ==
  ...
  if (tria.n_levels() > nsparam.mesh_adaptation.maximum_refinement_level)
    for (typename Triangulation<dim>::active_cell_iterator cell =
           tria.begin_active(nsparam.mesh_adaptation.maximum_refinement_level);
         cell != tria.end();
         ++cell)
      cell->clear_refine_flag();
  for (typename Triangulation<dim>::active_cell_iterator cell =
         tria.begin_active(nsparam.mesh_adaptation.minimum_refinement_level);
       cell !=
       tria.end_active(nsparam.mesh_adaptation.minimum_refinement_level);
       ++cell)
  ...
  VectorType tmp_m1(locally_owned_dofs, mpi_communicator);
  VectorType tmp_m2(locally_owned_dofs, mpi_communicator);
  VectorType tmp_m3(locally_owned_dofs, mpi_communicator);
...
NavierStokesBase<dim, VectorType, DofsType>::postprocess(bool firstIter)
{
  ...
  if (nsparam.post_processing.calculate_enstrophy)
    {
      double enstrophy = calculate_enstrophy(dof_handler,
                                             present_solution,
                                             nsparam.fem_parameters,
                                             mpi_communicator);

      enstrophy_table.add_value("time", simulation_control->get_current_time());
      enstrophy_table.add_value("enstrophy", enstrophy);

      // Display Enstrophy to screen if verbosity is enabled
      if (nsparam.post_processing.verbosity == Parameters::Verbosity::verbose)
        {
      ...
      if (simulation_control->get_step_number() %
          ...
          this_mpi_process == 0)
        {
          ...
          enstrophy_table.write_text(output);
        }
    }
  ...
  if (nsparam.post_processing.calculate_average_velocities)
    {
      ...
      if (simulation_control->get_current_time() >
        ...
        {
          average_velocities.calculate_average_velocities(
  ...
  if (nsparam.post_processing.calculate_kinetic_energy)
    {
      TimerOutput::Scope t(computing_timer, "kinetic_energy_calculation");
      double             kE = calculate_kinetic_energy(dof_handler,
                                           present_solution,
                                           nsparam.fem_parameters,
                                           mpi_communicator);
      kinetic_energy_table.add_value("time",
                                     simulation_control->get_current_time());
      ...
      if ((simulation_control->get_step_number() %
          ...
          this_mpi_process == 0)
        {
          ...
          kinetic_energy_table.write_text(output);
        }
    }
  ...
  if (nsparam.flow_control.enable_flow_control)
    {
      postprocessing_flow_rate(this->get_present_solution());
    }
  ...
  if (!firstIter)
    {
      ...
      if (nsparam.forces_parameters.calculate_force)
        {
          if (simulation_control->get_step_number() %
                nsparam.forces_parameters.calculation_frequency ==
              0)
            postprocessing_forces(present_solution);
          if (simulation_control->get_step_number() %
                nsparam.forces_parameters.output_frequency ==
              0)
            write_output_forces();
        }
      ...
      if (nsparam.forces_parameters.calculate_torque)
        {
          if (simulation_control->get_step_number() %
                nsparam.forces_parameters.calculation_frequency ==
              0)
            postprocessing_torques(present_solution);
          if (simulation_control->get_step_number() %
                nsparam.forces_parameters.output_frequency ==
              0)
            write_output_torques();
      ...
      if (nsparam.analytical_solution->calculate_error())
        {
          ...
          if (nsparam.simulation_control.method ==
              Parameters::SimulationControl::TimeSteppingMethod::steady)
            {
              ...
              error_table.add_value("error_velocity", error_velocity);
              error_table.add_value("error_pressure", error_pressure);
              ...
              error_table.add_value("total_time", total_time);
            }
          else
            {
          ...
          if (nsparam.analytical_solution->verbosity ==
  ...
  if (simulation_control->is_output_iteration())
    write_output_results(present_solution);
}
...
NavierStokesBase<dim, VectorType, DofsType>::set_nodal_values()
{
  ...
  VectorTools::interpolate(mapping,
                           dof_handler,
                           nsparam.initial_condition->uvwp,
                           newton_update,
...
NavierStokesBase<dim, VectorType, DofsType>::read_checkpoint()
{
  ...
  std::string        prefix = nsparam.restart_parameters.filename;
   ...
   */
  VectorType distributed_system(locally_owned_dofs, mpi_communicator);
  VectorType distributed_system_m1(locally_owned_dofs, mpi_communicator);
  VectorType distributed_system_m2(locally_owned_dofs, mpi_communicator);
  VectorType distributed_system_m3(locally_owned_dofs, mpi_communicator);
  ...
  if (nsparam.post_processing.calculate_average_velocities)
    {
      std::vector<VectorType *> sum_vectors = average_velocities.read(prefix);
  ...
  if (nsparam.post_processing.calculate_average_velocities)
    {
      ...
      data_out.add_data_vector(average_velocities.get_average_velocities(),
      ...
      data_out.add_data_vector(
        average_velocities.get_reynolds_normal_stresses(),
        reynolds_normal_stress_names,
        DataOut<dim>::type_dof_data,
        reynolds_normal_stress_data_component_interpretation);

      data_out.add_data_vector(
        average_velocities.get_reynolds_shear_stresses(),
  ...
  if (simulation_control->get_output_boundaries())
    {
      ...
      write_boundaries_vtu<dim>(
        data_out_faces, folder, time, iter, mpi_communicator);
    }
}
  ...
  if (nsparam.post_processing.calculate_average_velocities)
    {
      std::vector<const VectorType *> av_set_transfer =
        average_velocities.save(prefix);
