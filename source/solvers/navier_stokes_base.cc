      ...
      if constexpr (dim == 3)
        {
          ...
          parallel::distributed::Triangulation<patch_dim, spacedim> tria_slice(
            this->mpi_communicator);
          ...
          GridGenerator::merge_triangulations(
            {&tria_slice_left, &tria_slice_right}, tria_slice, 1.e-12, true);
          ...
          if (my_id == 0)
            {
              ...
              const unsigned int n_files = (group_files == 0) ?
                                             n_processes :
                                             std::min(group_files, n_processes);

              for (unsigned int i = 0; i < n_files; ++i)
                filenames.push_back(solution_name + "_slices" + "." +
                                    Utilities::int_to_string(iter, digits) +
                                    "." + Utilities::int_to_string(i, digits) +
                                    ".vtu");
