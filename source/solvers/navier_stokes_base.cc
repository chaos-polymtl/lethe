...
NavierStokesBase<dim, VectorType, DofsType>::box_refine_mesh()
{
    ...
    if (this->simulation_parameters.mesh_box_refinement.type == Parameters::MeshBoxRefinement::Type::gmsh)
    {
        if (this->simulation_parameters.mesh_box_refinement.simplex)
        {
            ...
            std::ifstream input_file(this->simulation_parameters.mesh_box_refinement.file_name);

            grid_in.read_msh(input_file);

            // By default uses the METIS partitioner.
            // A user parameter option could be made to chose a partitionner.
            GridTools::partition_triangulation(0,
                    basetria);
        ...
        // Dealii grids
    else if (this->simulation_parameters.mesh_box_refinement.type == Parameters::MeshBoxRefinement::Type::dealii)
    {
        if (this->simulation_parameters.mesh_box_refinement.simplex)
        {
            ...
            // initial refinement
            const int initial_refinement = this->simulation_parameters.mesh_box_refinement.initial_refinement;
