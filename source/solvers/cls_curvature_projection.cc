...
template <int dim>
void
CLSCurvatureProjection<dim>::assemble_system_matrix_and_rhs()
{
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              ...
              const double projected_cls_phase_gradient_norm =
                projected_cls_phase_gradient.norm() + tolerance;
              ...
              for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                {
                  ...
                  local_rhs(i) += grad_phi[i] *
                                  (projected_cls_phase_gradient /
                                   projected_cls_phase_gradient_norm) *
                                  JxW_vec[q];
                }
            }
...
template <int dim>
void
CLSCurvatureProjection<dim>::check_dependencies_validity()
{
...
template class CLSCurvatureProjection<2>;
template class CLSCurvatureProjection<3>;
