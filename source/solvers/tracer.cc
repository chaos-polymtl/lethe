...
template <int dim>
void
Tracer<dim>::setup_amg()
{
  TimerOutput::Scope t(this->computing_timer, "Setup AMG");

  // Constant modes for the tracer scalar field
  ...
  // The tracer operator is non-symmetric since the tracer is advected by the
  // fluid velocity
  ...
  const unsigned int n_cycles =
    simulation_parameters.linear_solver.at(PhysicsID::tracer).amg_n_cycles;
  const bool w_cycle =
    simulation_parameters.linear_solver.at(PhysicsID::tracer).amg_w_cycles;
...
Tracer<dim>::reinit_mortar_operators()
{
  ...
  this->mortar_coupling_evaluator =
    std::make_shared<ScalarCouplingEvaluation<dim, 1, double>>(
      *this->get_mapping(),
      *this->dof_handler,
      simulation_parameters.physical_properties_manager
