...
// template <int dim, typename number>
// void
// NavierStokesNonNewtonianStabilizedOperator<dim,
// number>::do_cell_integral_local(
//   FECellIntegrator &integrator) const
// {
//   if (this->enable_hessians_jacobian)
//     integrator.evaluate(EvaluationFlags::values | EvaluationFlags::gradients |
...
//       integrator.get_value(q); typename FECellIntegrator::gradient_type
//       gradient =
//         integrator.get_gradient(q);
//       typename FECellIntegrator::gradient_type hessian_diagonal;
...
//           // Time derivatives of previous solutions
//           Tensor<1, dim + 1, VectorizedArray<number>>
//           previous_time_derivatives; if (transient)
//             previous_time_derivatives =
//               this->time_derivatives_previous_solutions(cell, q);
...
//               for (unsigned int j = 0; j < dim; ++j)
//                 {
//                   shear_rate_magnitude += shear_rate[i][j] *
//                   shear_rate[j][i];
//                 }
//             }
...
//           // using the derived form, since it does not change with
//           rheological
//           // models
