...
template <int dim>
void
CLSPhaseGradientProjection<dim>::assemble_system_matrix_and_rhs()
{
      ...
      if (cell->is_locally_owned())
        {
          ...
          typename DoFHandler<dim>::active_cell_iterator cls_cell(
            &(*this->triangulation),
            cell->level(),
            cell->index(),
            &dof_handler_cls);
              ...
              for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                {
                  ...
                  local_rhs(i) +=
                    phi[i] * present_cls_phase_gradients[q] * JxW_vec[q];
                }
            }
...
{
  AssertThrow(this->subequations_interface.get_cls_solution().size() > 0,
              NoCLSSolution(this->subequations_interface.get_subequation_string(
                this->subequation_id)));
...
template class CLSPhaseGradientProjection<2>;
template class CLSPhaseGradientProjection<3>;
