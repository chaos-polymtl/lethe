...
template class HeatTransferAssemblerLaserVOF<2>;
template class HeatTransferAssemblerLaserVOF<3>;
...
template <int dim>
void
HeatTransferAssemblerALE<dim>::assemble_matrix(
  HeatTransferScratchData<dim> &scratch_data,
  StabilizedMethodsCopyData    &copy_data)
{
  ...
  for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      strong_residual_vec[q] +=
        -rho_cp * velocity_ale * scratch_data.temperature_gradients[q];

      // Strong residual jacobian calculation
      for (unsigned int j = 0; j < n_dofs; ++j)
        {
          strong_jacobian_vec[q][j] +=
            -rho_cp * velocity_ale * scratch_data.grad_phi_T[q][j];
        }
          ...
          for (unsigned int j = 0; j < n_dofs; ++j)
            {
              ...
              // Weak form for : -u_ALE * gradT
              local_matrix(i, j) +=
                -rho_cp * phi_T_i * velocity_ale * grad_phi_T_j * JxW;
            }
        }
...
template <int dim>
void
HeatTransferAssemblerALE<dim>::assemble_rhs(
  HeatTransferScratchData<dim> &scratch_data,
  StabilizedMethodsCopyData    &copy_data)
{
  const std::vector<double> &density       = scratch_data.density;
...
template class HeatTransferAssemblerALE<2>;
template class HeatTransferAssemblerALE<3>;
