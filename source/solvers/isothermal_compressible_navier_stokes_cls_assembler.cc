...
template <int dim>
void
GLSIsothermalCompressibleNavierStokesCLSAssemblerCore<dim>::assemble_matrix(
      ...
      const double tau =
        this->simulation_control->get_assembly_method() ==
          ...
          calculate_navier_stokes_gls_tau_steady(
            u_mag, dynamic_viscosity_for_stabilization / density_eq, h) :
          calculate_navier_stokes_gls_tau_transient(
            u_mag, dynamic_viscosity_for_stabilization / density_eq, h, sdt);
...
template <int dim>
void
GLSIsothermalCompressibleNavierStokesCLSAssemblerCore<dim>::assemble_rhs(
  const NavierStokesScratchData<dim>   &scratch_data,
  StabilizedMethodsTensorCopyData<dim> &copy_data)
{
  const std::vector<double> &viscosity_for_stabilization_vector =
...
template class GLSIsothermalCompressibleNavierStokesCLSAssemblerCore<2>;
template class GLSIsothermalCompressibleNavierStokesCLSAssemblerCore<3>;

template <int dim>
void
GLSIsothermalCompressibleNavierStokesCLSAssemblerBDF<dim>::assemble_matrix(
  const NavierStokesScratchData<dim>   &scratch_data,
  StabilizedMethodsTensorCopyData<dim> &copy_data)
{
  // Loop and quadrature information
...
template <int dim>
void
GLSIsothermalCompressibleNavierStokesCLSAssemblerBDF<dim>::assemble_rhs(
  const NavierStokesScratchData<dim>   &scratch_data,
  StabilizedMethodsTensorCopyData<dim> &copy_data)
{
  // Loop and quadrature information
...
template class GLSIsothermalCompressibleNavierStokesCLSAssemblerBDF<2>;
template class GLSIsothermalCompressibleNavierStokesCLSAssemblerBDF<3>;
