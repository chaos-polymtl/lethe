      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          const Tensor<1, dim> reactive_species_gradient =
            scratch_data.reactive_species_gradients[species][q];
          ...
          const double order =
            scratch_data.fe_values_reactive_species.get_fe().degree;

          const double vdcdd = (0.5 * h) * (velocity.norm() * velocity.norm()) *
                               pow(reactive_species_gradient.norm() * h, order);

          const double   tolerance = 1e-12;
          Tensor<1, dim> s         = velocity / (velocity.norm() + tolerance);
          Tensor<1, dim> r         = reactive_species_gradient /
                             (reactive_species_gradient.norm() + tolerance);
          ...
          const double d_vdcdd =
            ...
            pow(reactive_species_gradient.norm() * h, order - 1);
          ...
          for (unsigned int j = 0; j < n_dofs; ++j)
            {
              const Tensor<1, dim> grad_phi_T_j =
                scratch_data.grad_phi[species][q][j];
              const double laplacian_phi_T_j =
                scratch_data.laplacian_phi[species][q][j];
          ...
          for (unsigned int i = 0; i < n_dofs; ++i)
            {
              const auto phi_T_i      = scratch_data.phi[species][q][i];
              const auto grad_phi_T_i = scratch_data.grad_phi[species][q][i];
              ...
              for (unsigned int j = 0; j < n_dofs; ++j)
                {
                  ...
                  local_matrix(i, j) +=
                    (vdcdd * scalar_product(grad_phi_T_i,
                                            dcdd_factor * grad_phi_T_j) +
                     d_vdcdd * grad_phi_T_j.norm() *
                       scalar_product(grad_phi_T_i,
                                      dcdd_factor *
                                        reactive_species_gradient)) *
                    JxW;
                }
            }
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          const Tensor<1, dim> tracer_gradient =
            scratch_data.reactive_species_gradients[species][q];
          const double tracer_laplacian =
            scratch_data.reactive_species_laplacians[species][q];
          const Tensor<1, dim> velocity = scratch_data.velocity_values[q];
          ...
          for (unsigned int i = 0; i < n_dofs; ++i)
            {
              ...
              local_rhs(i) -= (diffusivity * grad_phi_T_i * tracer_gradient +
                               ...
                               scratch_data.source[species][q] * phi_T_i) *
                              JxW;

              local_rhs(i) -=
                tau * (strong_residual_vec[q] * (grad_phi_T_i * velocity)) *
                JxW;

              local_rhs(i) -=
                vdcdd *
                scalar_product(grad_phi_T_i, dcdd_factor * tracer_gradient) *
                JxW;
            }
        } // end loop on quadrature points
    }     // end loop on species
