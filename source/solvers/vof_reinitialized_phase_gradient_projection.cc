...
template <int dim>
void
VOFReinitializedPhaseGradientProjection<dim>::assemble_system_matrix_and_rhs()
{
  // Reinitialize system matrix and right-hand side (rhs)
  this->system_matrix = 0;
  this->system_rhs    = 0;

  // Get Algebraic interface reinitialization DoFHandler
  const DoFHandler<dim> *dof_handler_algebraic_reinitialization =
  ...
  // Initialize reinitialized phase fraction gradient solution array
  std::vector<Tensor<1, dim>> present_reinitialized_phase_gradients(n_q_points);
      ...
      if (cell->is_locally_owned())
        {
          ...
          // Get algebraic interface reinitialization cell iterator
              ...
              for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                {
                  ...
                  local_rhs(i) += phi[i] *
                                  present_reinitialized_phase_gradients[q] *
                                  JxW_vec[q];
                }
            }
...
template class VOFReinitializedPhaseGradientProjection<2>;
template class VOFReinitializedPhaseGradientProjection<3>;
