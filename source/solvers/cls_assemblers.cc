...
template <int dim>
void
CLSAssemblerCore<dim>::assemble_matrix(const CLSScratchData<dim> &scratch_data,
                                       StabilizedMethodsCopyData &copy_data)
{
  ...
  // Add a small diffusivity, used to artificially diffuse CLS
  const double diffusivity = this->cls_parameters.diffusivity;
          ...
          for (unsigned int j = 0; j < n_dofs; ++j)
            {
              ...
              // Add compressibility term if the CLS is compressible
              if (compressible)
                local_matrix(i, j) +=
...
template <int dim>
void
CLSAssemblerCore<dim>::assemble_rhs(const CLSScratchData<dim> &scratch_data,
                                    StabilizedMethodsCopyData &copy_data)
{
  // Scheme and physical properties
  const auto method = this->simulation_control->get_assembly_method();
...
template class CLSAssemblerCore<2>;
template class CLSAssemblerCore<3>;


template <int dim>
void
CLSAssemblerBDF<dim>::assemble_matrix(const CLSScratchData<dim> &scratch_data,
                                      StabilizedMethodsCopyData &copy_data)
{
  // Loop and quadrature information
...
template <int dim>
void
CLSAssemblerBDF<dim>::assemble_rhs(const CLSScratchData<dim> &scratch_data,
                                   StabilizedMethodsCopyData &copy_data)
{
  // Loop and quadrature information
  const auto        &JxW        = scratch_data.JxW;
...
template class CLSAssemblerBDF<2>;
template class CLSAssemblerBDF<3>;


template <int dim>
void
CLSAssemblerDCDDStabilization<dim>::assemble_matrix(
...
template <int dim>
void
CLSAssemblerDCDDStabilization<dim>::assemble_rhs(
  const CLSScratchData<dim> &scratch_data,
  StabilizedMethodsCopyData &copy_data)
{
  // Loop and quadrature information
...
template class CLSAssemblerDCDDStabilization<2>;
template class CLSAssemblerDCDDStabilization<3>;


template <int dim>
void
CLSAssemblerDGCore<dim>::assemble_matrix(
  const CLSScratchData<dim> &scratch_data,
  StabilizedMethodsCopyData &copy_data)
{
  // Loop and quadrature information
...
template <int dim>
void
CLSAssemblerDGCore<dim>::assemble_rhs(const CLSScratchData<dim> &scratch_data,
                                      StabilizedMethodsCopyData &copy_data)
{
  // Loop and quadrature information
  const auto        &JxW_vec    = scratch_data.JxW;
...
template class CLSAssemblerDGCore<2>;
template class CLSAssemblerDGCore<3>;



template <int dim>
void
CLSAssemblerSIPG<dim>::assemble_matrix(const CLSScratchData<dim> &scratch_data,
                                       StabilizedDGMethodsCopyData &copy_data)
{
  const FEInterfaceValues<dim> &fe_iv    = scratch_data.fe_interface_values_cls;
        ...
        for (unsigned int j = 0; j < n_dofs; ++j)
          {
            ...
            // There is no penalty coefficient here since the CLS equation
...
template <int dim>
void
CLSAssemblerSIPG<dim>::assemble_rhs(const CLSScratchData<dim>   &scratch_data,
                                    StabilizedDGMethodsCopyData &copy_data)
{
  const FEInterfaceValues<dim> &fe_iv    = scratch_data.fe_interface_values_cls;
  const auto                   &q_points = fe_iv.get_quadrature_points();
