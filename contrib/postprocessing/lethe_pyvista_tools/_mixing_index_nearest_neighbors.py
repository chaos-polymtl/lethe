...
def mixing_index_nearest_neighbors_loop(self, i):
    ...
    n_equal_neighbors_per_particle = np.sum(np.equal(df[self.reference_array][:, None], list_neighbor_reference_array), axis = 1)
...
def mixing_index_nearest_neighbors(self, n_neighbors = 15, reference_array = "particle_color", mixing_index_array_name = "mixing_index"):
    ...
    self.pbar = tqdm(total = len(self.list_vtu), desc = "Calculating mixing index")
    #for i in range(len(self.list_vtu)):
    

    pool.map(mixing_index_nearest_neighbors_loop, range(len(self.list_vtu)))