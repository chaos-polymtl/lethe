  ...
  for (unsigned int i_particle = 0; i_particle < 4; i_particle++)
    {
      ...
      double alpha = particle_detector_interactions.get_alpha(n_alpha, n_theta);
      double theta = particle_detector_interactions.get_theta(n_alpha, n_theta);
      deallog << "Particle position " << i_particle << " : alpha = " << alpha
