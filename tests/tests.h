...
/**
 * Go through the input stream @p in and filter out binary data for the key @p
 * key . The filtered stream is returned in @p out.
 */
void filter_out_xml_key(std::istream &in, const std::string &key,
                        std::ostream &out) {
  std::string line;
  bool found = false;
...
template <typename number> number get_real_assert_zero_imag(const number &a) {
  return a;
}
 ...
 */
template <typename Number> Number nonoverflow_add(Number a, Number b) {
 ...
 */
template <class IT> unsigned int checksum(const IT &begin, const IT &end) {
...
#define check_solver_within_range(SolverType_COMMAND, CONTROL_COMMAND,         \
 ...
 * of the core count.
 */
inline unsigned int testing_max_num_threads() { return 3; }
...
void init_cuda(const bool use_mpi = false) {
