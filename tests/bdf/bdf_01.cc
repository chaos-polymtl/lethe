...
main(int argc, char* argv[])
{
  try
  {
    ...
    if(!approximatelyEqual(order1_coefficients[0],10.,1e-8)) throw std::runtime_error("Error order 1 term 0");
    if(!approximatelyEqual(order1_coefficients[1],-10.,1e-8)) throw std::runtime_error("Error order 1 term 1");
    deallog << "Order 1 : " << order1_coefficients << std::endl;

    Vector<double> order2_coefficients=bdf_coefficients(2,dt);
    if(!approximatelyEqual(order2_coefficients[0],13.3333333333333,1e-8))  throw std::runtime_error("Error order 2 term 0");
    if(!approximatelyEqual(order2_coefficients[1],-15.0000000000000,1e-8)) throw std::runtime_error("Error order 2 term 1");
    if(!approximatelyEqual(order2_coefficients[2],1.66666666666667,1e-8))  throw std::runtime_error("Error order 2 term 2");
    deallog << "Order 2 : " << order2_coefficients << std::endl;

    Vector<double> order3_coefficients=bdf_coefficients(3,dt);
    if(!approximatelyEqual(order3_coefficients[0],15.0000000000000   , 1e-8))  throw std::runtime_error("Error order 3 term 0");
