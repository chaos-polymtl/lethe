 ...
 * when the QCM Gaussian filter is used. Mirrors particle_projector_01 but with
 * qcm_filter_type = gaussian. The constant void fraction is defined by
 * homogenously placing particles in a grid within a cubic triangulation and
 * averaging at a significantly larger volume than the volume of the particles.
 */
...
void
test_void_fraction_qcm_gaussian(const unsigned int fe_degree,
                                const unsigned int number_quadrature_points)
{
  ...
  // Setup default VoidFractionParameters, then switch to the Gaussian filter.
  // The half-diameter (here 0.25) is reinterpreted as the Gaussian standard
  // deviation sigma.
  ...
  void_fraction_parameters->qcm_filter_type =
    Parameters::QCMFilterType::gaussian;
  ...
  try
    {
      ...
      deallog
        << "Void fraction (gaussian filter): fe_degree=1    number_quadrature_points=2"
        << std::endl;
      test_void_fraction_qcm_gaussian(1, 2);
      deallog
        << "Void fraction (gaussian filter): fe_degree=1    number_quadrature_points=3"
        << std::endl;
      test_void_fraction_qcm_gaussian(1, 3);
      deallog
        << "Void fraction (gaussian filter): fe_degree=2    number_quadrature_points=3"
        << std::endl;
      test_void_fraction_qcm_gaussian(2, 3);
    }
