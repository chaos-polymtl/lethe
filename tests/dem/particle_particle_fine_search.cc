...
template <int dim> void test() {
  ...
  fine_search_obejct.pp_Fine_Search(
      local_contact_pair_candidates, ghost_contact_pair_candidates,
  ...
  for (auto adjacent_particles_iterator = local_adjacent_particles.begin();
    ...
    auto information_iterator = (&adjacent_particles_iterator->second)->begin();
    while (information_iterator !=
           (&adjacent_particles_iterator->second)->end()) {
      deallog << "The particle pair in contact are particles: "
              << information_iterator->second.particle_one->get_id() << " and "
