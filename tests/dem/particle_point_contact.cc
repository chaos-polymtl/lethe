...
template <int dim> void test() {
  ...
  dem_parameters.physicalProperties.restitution_coefficient_particle = 0.95;
  dem_parameters.physicalProperties.restitution_coefficient_wall = 0.95;
  dem_parameters.physicalProperties.friction_coefficient_particle = 0.05;
  dem_parameters.physicalProperties.friction_coefficient_wall = 0.05;
  ...
  boundary_cells_object.find_particle_point_and_line_contact_cells(
      boundary_cells_with_faces, tr, boundary_cells_with_lines,
      boundary_cells_with_points);
  ...
  for (double time = 0; time < 0.2; time += dt) {
    ...
    contact_candidates = broad_search_object.find_Particle_Point_Contact_Pairs(
