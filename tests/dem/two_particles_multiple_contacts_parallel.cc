...
void update_contact_containers(
...
template <int dim> void test() {
  ...
  for (unsigned int iteration = 0; iteration < step_end; ++iteration) {
    ...
    locate_local_particles_in_cells(
        particle_handler, local_particle_container, ghost_particle_container,
    ...
    broad_search_object.find_particle_particle_contact_pairs(
        particle_handler, &local_neighbor_list, &ghost_neighbor_list,
    ...
    fine_search_object.particle_particle_fine_search(
        local_contact_pair_candidates, ghost_contact_pair_candidates,
        cleared_local_adjacent_particles, cleared_ghost_adjacent_particles,
        local_particle_container, neighborhood_threshold);

    // Calling non-linear force
    nonlinear_force_object.calculate_pp_contact_force(
        cleared_local_adjacent_particles, cleared_ghost_adjacent_particles,
        dem_parameters.physical_properties, dt);
      ...
      if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 1) {
        ...
        for (auto particle = particle_handler.begin();
          ...
          if (particle->get_id() == 0) {
            ...
            deallog << "The exerted force on particle " << particle->get_id()
                    << " at step " << iteration << " is: "
