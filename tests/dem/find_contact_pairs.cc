...
template <int dim> void test() {
  ...
  broad_search_object.find_PP_Contact_Pairs(
      particle_handler, cell_neighbor_list, broad_search_pairs);

  // Output
  for (auto pairs_iterator = broad_search_pairs.begin();
       pairs_iterator != broad_search_pairs.end(); ++pairs_iterator) {
    auto pairs_content = &pairs_iterator->second;
    deallog << "A pair is detected: particle " << pairs_content->first->get_id()
            << " and particle " << pairs_content->second->get_id() << std::endl;
  }
}
