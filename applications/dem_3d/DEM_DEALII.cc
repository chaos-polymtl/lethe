...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::tuple<std::string,int>> properties(DEMparam.outputProperties.numProperties);
    ...
    properties[19] = std::make_tuple("mass",1); 	properties[20] = std::make_tuple("MOI",1);
	...
	Particles::ParticleHandler<3,3> particle_handler(tr, mappinggg , DEMparam.outputProperties.numProperties);
	...
	std::vector<std::tuple<std::pair<Particles::ParticleIterator<3,3>, int>, Point<3>, Point<3>, double, double, double, Point<3>, double>> pwContactInfo;


	std::vector<std::tuple<int, Triangulation<3>::active_cell_iterator, int, Point<3>, Point<3>>> boundaryCellInfo;
	...
	while (DEM_step < readInput.tInsertion)
	{
		...
		if (fmod(DEM_step,readInput.writeFrequency) == 1)
			{
				...
				visObj.build_patches(particle_handler, DEMparam.outputProperties.numFields, DEMparam.outputProperties.numProperties, properties);
		...
		iter1.engine(nPart, particle_handler, tr, DEM_step, DEM_time, readInput, cellNeighbor, contactInfo, boundaryCellInfo, pwContactInfo);
