...
template <int dim> class ExactSolutionMMS3D : public Function<dim> {
public:
...
void ExactSolutionMMS3D<dim>::vector_value(const Point<dim> &p,
  ...
  double y = p[1];
  const double z = p[2];
  values(0) = sin(a * x) * sin(a * x) * cos(a * y) * sin(a * y) * cos(a * z) *
              sin(a * z);
  values(1) = cos(a * x) * sin(a * x) * sin(a * y) * sin(a * y) * cos(a * z) *
              sin(a * z);
  values(2) = -2 * cos(a * x) * sin(a * x) * cos(a * y) * sin(a * y) *
              sin(a * z) * sin(a * z);
}

template <int dim> class MMS3DNavierStokes : public GLSNavierStokesSolver<dim> {
public:
  MMS3DNavierStokes(NavierStokesSolverParameters<dim> nsparam,
...
template <int dim> void MMS3DNavierStokes<dim>::runMMS_3D() {
  ...
  if (this->this_mpi_process == 0) {
    ...
    for (unsigned int i = 0; i < ErrorLog.size(); ++i) {
      output_file << i + initialSize << " " << ErrorLog[i] << " " << wallTime[i]
                  << std::endl;
    }
    output_file.close();
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    MMS3DNavierStokes<3> problem(NSparam, NSparam.femParameters.velocityOrder,
