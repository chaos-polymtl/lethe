...
template <int dim>
void
ExactSolutionMMS3D<dim>::vector_value(const Point<dim> &p,
                                      Vector<double> &  values) const
{
  ...
  const double z = p[2];
  values(0) =
    sin(a * x) * sin(a * x) * cos(a * y) * sin(a * y) * cos(a * z) * sin(a * z);
  values(1) =
    cos(a * x) * sin(a * x) * sin(a * y) * sin(a * y) * cos(a * z) * sin(a * z);
...
template <int dim>
void
MMS3DNavierStokes<dim>::runMMS_3D()
{
  ...
  if (this->this_mpi_process == 0)
    {
      ...
      for (unsigned int i = 0; i < ErrorLog.size(); ++i)
        {
          output_file << i + initialSize << " " << ErrorLog[i] << " "
                      << wallTime[i] << std::endl;
        }
      output_file.close();
    }
  ...
  try
    {
      ...
      MMS3DNavierStokes<3> problem(NSparam,
