...
template <int dim>
void
ConstantXForce<dim>::vector_value(const Point<dim> & /*p*/,
...
template <int dim>
void
ExactSolutionPoiseuille<dim>::vector_value(const Point<dim> &p,
                                           Vector<double> &  values) const
{
  assert(dim == 2);
  const double H  = 1.;
