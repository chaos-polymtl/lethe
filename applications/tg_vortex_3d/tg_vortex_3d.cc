...
template<int dim>
void TaylorGreenVortex<dim>::read_ke()
{
...
template<int dim>
void TaylorGreenVortex<dim>::read_enstrophy()
{
      ...
      if(this->this_mpi_process==0)
      {
        ...
        this->pcout << "Kinetic energy : " <<  kE << std::endl;
        {
          ...
          for (unsigned int i=0 ; i < ke_values.size() ; ++i)
            output_file<<std::setprecision(12) <<time[i]<<" "<<std::setprecision(12) <<ke_values[i]<< std::endl;
          output_file.close();
        }
        ...
        this->pcout << "Enstrophy  : " << enstrophy << std::endl;
        {
          ...
          for (unsigned int i=0 ; i < enstrophy_values.size() ; ++i)
            output_file<< std::setprecision(12) << time[i]<< " "<< std::setprecision(12) << enstrophy_values[i]<< std::endl;
          output_file.close();
        }
      }
    }
    ...
    if (argc < 2)
      {
        ...
        NSparam.parse(prm);
        TaylorGreenVortex<3> problem_3d(NSparam,NSparam.femParameters.velocityOrder,NSparam.femParameters.pressureOrder);
        problem_3d.run3DTGV();
