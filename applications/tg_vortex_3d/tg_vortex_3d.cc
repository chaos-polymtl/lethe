...
public:
  TaylorGreenVortex(NavierStokesSolverParameters<dim> nsparam,
    ...
    if (this->nsparam.restartParameters.restart &&
...
template <int dim> void TaylorGreenVortex<dim>::read_ke() {
...
template <int dim> void TaylorGreenVortex<dim>::read_enstrophy() {
...
// 3D TGV case
template <int dim> void TaylorGreenVortex<dim>::run3DTGV() {
        ...
        this->pcout << "Kinetic energy : " << kE << std::endl;
        {
          ...
          for (unsigned int i = 0; i < ke_values.size(); ++i)
            output_file << std::setprecision(12) << time[i] << " "
                        << std::setprecision(12) << ke_values[i] << std::endl;
          output_file.close();
        }
